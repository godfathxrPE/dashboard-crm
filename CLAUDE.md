...
Supabase ref: `uoiavcabxgdjugzryrmj`, Postgres 17.6. Применённые миграции — **001–084**,
следующая свободная — **085** (060 зарезервирована и не занята — не возвращаться к ней).
Edge-функция `ai-run` — **version 4**.
...
4. **Разведка перед правкой.** Живая БД — источник истины по схеме, не папка миграций и не
   `docs/schema.md`: файл синхронизирован до 082, но отстаёт сразу же, как только
   применяется новая миграция.
