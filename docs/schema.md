...
> edge не требует. Следующая свободная после неё — **115**;
...
### Лента сущности (112 applied `20260808204308` + 113 applied `20260808222500` + 114 **applied 2026-08-09 `20260809083732`**) — сборка на сервере, страницами, с фильтром по видам

- **`public.entity_timeline(p_entity_type text, p_entity_id uuid, p_before timestamptz
  default null, p_before_id text default null, p_limit int default 50,
  p_kinds text[] default null)`**
  → `table(ts timestamptz, id text, source text, kind text, actor_id uuid, ref_type text,
  ref_id uuid, payload jsonb)`. `language sql`, **STABLE**, `security invoker`,
  `search_path=public,pg_temp`, `revoke public/anon` → `grant execute to authenticated`.
...
- ⚠️ **Сигнатура заменялась ТРИЖДЫ, и каждый раз через `drop`, а не перегрузкой.**
  ...
  пяти-, и шестиаргументному кандидату. Гранты после `drop`+`create` не наследуются и
  повторены явно (в 114 — уже с шестью аргументами в сигнатуре).
- **`p_kinds` (114) — фильтр по видам, `null` = все.** Словарь из семи значений:
  `call | meeting | task | project | activity | ai_run` + производный **`note`** — срез
  внутри `activity_log` по `event_type in ('comment_added')`, а не отдельный источник
  (`kind` у такого события остаётся `activity`). Вид, которого нет в списке, схлопывает
  предикат своей ветки в `false`, и планировщик отбрасывает её как one-time filter, не
  читая таблицу. Значение вне словаря не ошибка — оно просто не включает ни одной ветки.
  ⚠️ **Список типов заметок ПРОДУБЛИРОВАН** в SQL и в `NOTE_EVENT_TYPES`
  (`src/lib/utils/activity-events.ts`): SQL не умеет импортировать TS-константу. При
  добавлении типа заметки править **оба места** — разойдутся молча.
...
- **Не в S-TL-3:** `org_timeline()` и замена «Последних действий» / `ActivityDrawer`
  (`useRecentActivity` читает `activity_log` напрямую, то есть org-лента в проде
  неполна по построению) — отдельным спринтом S-TL-4. Шесть новых источников (`stage_transitions`, `messages`,
