...
> ⚠️ **116 — ОТМЕНЕНА, не применялась** (2026-08-10, S-TAILS-1): популяция, под
> которую писался предикат (308 задач-сирот от неудавшихся прогонов
> `MigrationTool`), вычищена вручную из UI 2026-08-09 — в `activity_log` за тот
> день 387 записей `entity_deleted` по задачам, последняя 19:01 UTC; повторная
> сверка предиката на гейте дала **0 строк**. Гвард `if v_count <> 308 then raise`
> сработал бы ровно как задумано: уронил бы транзакцию, а не удалил «примерно то
> же самое», — и оставил бы в ledger след неудачной миграции. Файл
> `116_cleanup_orphan_import_tasks.sql` удалён из репозитория, номер 116 остаётся
> дырой и **не переиспользуется** (как 047 и 060 — правило «идти вперёд, не
> возвращаться»). Популяции **A** (осиротевшие задачи со связями) миграция и не
> касалась: на проде 103 задачи без `project_id`, среди них планы клиентов с
> компанией и контактом — решение по ним отдельное и здесь не принято.
> **117 `117_lead_work_fields.sql`** — двенадцать колонок работы
...
>
> **120 (S-LEAD-HUB-2a) — ПРИМЕНЕНА ГЕЙТОМ 2026-08-10, версия `20260810104424`**
> (`120_lead_timeline.sql`). Пятое значение `p_entity_type = 'lead'` у `entity_timeline`:
> предикат лида в трёх ветках из шести (`calls`/`tasks`/`activity_log`) и `lead`
> последним в `parent_type`/`parent_id`. **`create or replace` БЕЗ `drop`** — ни список
> аргументов, ни набор возвращаемых колонок не меняются (⇒ ни `PGRST203`, ни `42P13`),
> гранты сохраняются. Таблиц, колонок, enum, индексов и cron не трогает ⇒ **реген типов
> не нужен**: сигнатура прежняя. Редеплоя edge не требует.
> **Протокол гейта 120 (2026-08-10).** Тело сверено с живым `pg_get_functiondef`
> побайтово по CTE: `src_meetings` / `src_projects` / `src_ai` и служебные `n`,
> `kind_types`, `scope_projects`, `scope_children` — md5 совпал с точностью до символа;
> keyset (`p_before` / `p_before_id`, 8 вхождений), финальный union, `order by` и
> `limit` не тронуты; ни один существующий предикат не пропал — все изменения
> аддитивные. **Org-лента до и после apply идентична**: `activity ∅ 176`,
> `activity project 5`, `task company 17`, `task project 2` — ноль расхождений.
> Ленты сущностей живы (project 6, contact 1). Функция осталась SECURITY INVOKER,
> ACL прежний (`authenticated` execute = true, `anon` = false) ⇒ advisors не трогаем.
> Сквозной смок (транзакция с rollback): лид + звонок + задача + смена статуса →
> лента лида отдаёт три вида (`call`, `task`, `activity/lead_status_changed`), у всех
> `parent_type='lead'`; фильтр `p_kinds=['call']` работает; чужой лид отдаёт 0 строк.
> **После `convert_lead` родителем звонка и задачи становится `project`** (и `parent_id`
> равен id созданной сделки) — порядок «лид последним» в `case`/`coalesce` подтверждён
...
> а он общий пул и его разбирают несколько человек. RLS realtime уважает
> (`leads_select` org-wide), `REPLICA IDENTITY` не меняется (default/PK хватает: клиент
> инвалидирует ключ целиком, old-строка ему не нужна). Таблиц, колонок, функций и
> политик не трогает ⇒ **реген типов не нужен**.
> ⚠️ Находка разведки, в 121 не входившая (в publication нет `contacts`, `companies`
> и `activity_log`, хотя четыре хука на них подписаны), закрыта миграцией **122** (applied 2026-08-10).
>
> **122 (S-TAILS-1) — ПРИМЕНЕНА ГЕЙТОМ 2026-08-10, версия `20260810121513`**
> **Протокол гейта 122 (2026-08-10).** После apply в publication ровно четыре
> ключевые таблицы: `activity_log, companies, contacts, leads`; REPLICA IDENTITY у
> всех трёх новых — `d` (default/PK), как и планировалось; всего в publication 20
> таблиц. **Опасение «триггеры аудита пишут в `activity_log` чаще людей» ФАКТАМИ НЕ
> ПОДТВЕРДИЛОСЬ** — распределение за 30 дней: `entity_deleted` 443, `task_created`
> 412, `task_completed` 70, `project_updated` 48 (это и есть аудит полей 087),
> `stage_changed` 16, `stage_transition_committed` 11, `automation_fired` 9,
> `ai_summary_generated` 3. Аудит даёт ~5% потока, средняя нагрузка ≈ 34 записи в
> сутки (≈ 1–2 в час) — дровер этого не заметит. Реальный всплеск даёт не аудит, а
> МАССОВАЯ ОПЕРАЦИЯ ПОЛЬЗОВАТЕЛЯ: пиковая минута за неделю — 308 строк
> (bulk-удаление задач 2026-08-09 19:01 UTC), вторая — 79. Их сглаживает дебаунс
> 150 мс в `use-realtime`. Вывод: сужать `queryKey` в `use-activity-log`
> преждевременно — оставить как есть и вернуться, если всплывёт на живой работе. (`122_realtime_core_tables.sql`; номер
> сверен запросом к `schema_migrations` 2026-08-10 — последняя применённая
> `20260810114146 leads_realtime`; отменённая 116 номера не занимает). Три строки:
> `contacts`, `companies` и `activity_log` добавляются в publication
> `supabase_realtime`. Клиент подписан на них давно — `contacts`/`companies` с S-R2,
> `activity_log` с 042 (`use-contacts.ts:151`, `use-companies.ts:137`,
> `use-activity-log.ts:13`, `use-entity-timeline.ts:212`), — но publication их не
> отдавала: **канал открывался, событий не приходило никогда, и молчал он тихо**.
> Заметным дефект стал после 121: лид обновляется живьём, контакт рядом — нет.
> Кода миграция не требует, подписки уже написаны и корректны.
> ⚠️ **Эффект шире трёх хуков:** `activity_log` — источник ленты сущности, org-ленты
> дашборда и правого дровера; после apply они станут живыми у всех, кто держит
> страницу открытой. Это цель, но `activity_log` пишут ещё и триггеры аудита (087) с
> частотой выше пользовательских действий ⇒ на смоке отдельно смотреть частоту
> перерисовки дровера. Заметна — сужать `queryKey` инвалидации в `use-activity-log`,
> **не откатывать миграцию**. `REPLICA IDENTITY` не меняется (default/PK хватает:
> клиент инвалидирует ключ целиком). RLS realtime уважает — подписчик получает
> события только по строкам своей SELECT-политики. Таблиц, колонок, функций и политик
> не трогает ⇒ **реген типов не нужен**, advisors не меняются (публикация не объект RLS).
> Следующая свободная после неё — **123**;
>
> **123 (S-LEAD-CARRY-1) — applied 2026-08-12** (`123_lead_convert_carryover.sql`,
> номер сверен запросом к `schema_migrations` 2026-08-11 — последняя применённая
> `20260810121513 realtime_core_tables`). версия в ledger — `20260812060950 lead_convert_carryover`. Применена гейтом Cowork 2026-08-12;
> тело в БД сверено с файлом по md5 нормализованного (без комментариев) body — совпало. Три вещи в одном файле, все — про то, что **конверсия
> лида перестаёт терять квалификацию**:
> **(1)** `companies.chz_groups text[]` — подтверждённый маркировочный профиль КОМПАНИИ.
> Профиль это свойство компании (что она производит), а не разовой сделки: без колонки
> через год на второй сделке его выясняли заново. Не заменяет гипотезу по ОКВЭД —
> см. таблицу `companies` выше и `resolveChzProfile`.
> **(2)** `leads_decision_role_check` — зеркало `deal_stakeholders_role_chk`; домен
> закрывается на входе, потому что с 123 значение уезжает в таблицу с закрытым CHECK.
> **(3)** `convert_lead` **v3** — три переноса (`decision_role` → `deal_stakeholders`,
> `budget_status` → `pinned_note`, `chz_groups` → `companies`), разбор в разделе `leads`.
> ⚠️ **Бэкфилла конвертированных лидов НЕТ**: на 2026-08-11 их ноль — переносить нечего.
> ⚠️ **Реген типов в спринте не делался** (миграции в БД нет): колонка добрана стабом
> `CompaniesChzStub` в `src/types/database.ts` и полем `chz_groups?` в рукописном
> `Company`/`CompanyInsert` (`src/lib/hooks/use-companies.ts`). **Стаб обязан быть снят
> на гейте** после apply + `scripts/gen-types.sh` — оставленный, он переживает миграцию
> молча и продолжает врать про схему. `supabase.gen.ts` спринтом не тронут.
> ⚠️ **2026-08-12: миграция применена, реген типов ещё НЕ сделан.** Гейт Cowork ходит в
> БД через MCP и не имеет сети для `npx` — `scripts/gen-types.sh` гоняется на машине
...
> дедупа задач нет и не планируется — две одинаковые задачи законны.
> **(2)** `tg_apply_capture` **v2** — `create or replace` (аргументы и тип возврата
> прежние ⇒ `drop function` не нужен). Тело взято из `pg_get_functiondef` живой функции
> и **строго аддитивно**: изменены ровно две существующие строки — гард `p_kind`
> расширен на `'task'`, а `else` company-ветки стал `elsif v_kind = 'task' then … else`.
> Новая ветка: валидация ссылок из payload на принадлежность `draft.org_id`
> (`assigned_to` → `memberships`, `project_id` → `projects`, `company_id` → `companies`;
> не прошедшая **обнуляется**, а не роняет вставку) → INSERT в `tasks` с `lane='next'`,
> `created_by = p_actor_id`, явным `org_id` и приоритетом, сверенным со списком значений
> enum. Плюс адресный гард: черновик `kind='task'` нельзя применить кнопкой выбора
> ветки (`p_kind` ≠ `'task'` ⇒ 22023) — иначе payload без ключа `contact` создал бы
> контакт с пустым именем молча.
> ⚠️ **`lane='next'` при непустом `project_id` переписывается триггером
> `trg_aa_resolve_board`** — он подбирает колонку по `lane_to_category('next')` =
> `'backlog'` и ставит `lane := category_to_lane(категория колонки)`. Для `backlog` это
> снова `'next'`, и на 2026-08-19 backlog-колонка есть у **всех 17** клиентских
> проектов ⇒ фактический lane сохраняется. На доске без такой колонки берётся первая по
> `position`, и lane станет её категорией — это штатное поведение доски, ему не мешать.
> ⚠️ **Новых колонок, таблиц и политик нет ⇒ реген типов диффа дать не должен.**
> Появившийся дифф — находка, а не рутина.
> **125 (S-CONTACT-COMPANY) — applied `20260821104527 contact_company_capture`** (гейт
> Cowork, 2026-08-21; файл `125_contact_company_capture.sql`). Advisors после apply —
> ноль новых находок относительно baseline (4 INFO `rls_enabled_no_policy` на `telegram_*`
> + прежний набор WARN 0029; `tg_apply_capture` в 0029 не появилась). Контакт из Telegram привязывается к компании.
> `tg_apply_capture` **v3** — `create or replace` (аргументы и тип возврата прежние ⇒
> `drop function` не нужен). Тело взято из `pg_get_functiondef` живой функции 2026-08-21
> и сверено с файлом 124: нормализованные тела **совпали побайтно** (185 строк,
> md5 `82325aa872abb4ec0b23d542d01af40a`). Правка **строго аддитивна и затрагивает одну
> ветку**: в `contact` ПОСЛЕ вставки контакта появился блок привязки, ветки `company` и
> `task` не тронуты, ни одной существующей строки не изменено.
> ⚠️ **Связь контакт↔компания — НЕ колонка.** У `contacts` нет `company_id`; связь живёт
> в junction `contact_company` (`contact_id, company_id, role, is_primary, org_id`), и до
> этой миграции бот в неё не писал вовсе — разобранное название компании оставалось
> словом в `contacts.notes`.
> Блок: `company_id` из payload → гард принадлежности `draft.org_id`
> (`companies`; не прошёл ⇒ **обнуляется**, а не роняет вставку) → INSERT в
> `contact_company` с `role` = должность контакта и `is_primary = true`.
> ⚠️ Вставка обёрнута в `begin … exception when unique_violation then null; end` —
> обработчик на ОПЕРАЦИИ, а не на теле функции (тот же приём, что `duplicate_inn` в 111):
> на таблице висит `unique (contact_id, company_id)`, контакт к этому моменту уже создан,
> и падение на привязке оставило бы живую запись в `contacts` при неприменённом черновике.
> ⚠️ **Новых колонок, таблиц и политик нет ⇒ реген типов диффа дать не должен.**
> Возврат функции не меняется. Парная правка вне БД: `ai-capture` (поле `company_hint` в
> схеме интента `contact` + правило «название организации из остатка — не в `notes`») и
> `telegram-webhook` (резолв цитаты существующим `resolveCompany`, строка «Компания» в
> карточке подтверждения) — **обе функции требуют редеплоя**.
> **126 (S-EXPORT-1) — файл `126_org_export.sql` НАПИСАН, НЕ ПРИМЕНЁН** (ждёт гейта).
> Номер сверен запросом к `supabase_migrations.schema_migrations` 2026-08-22: последняя
> применённая — `20260821104527 contact_company_capture` (125) ⇒ 126. Выгрузка данных
> организации в JSON одним вызовом: новая функция **`export_org_data(uuid) → jsonb`**.
> ⚠️ **`SECURITY INVOKER`** — третий такой случай в проекте после `entity_timeline`
> (112) и по той же причине: изоляция org держится на RLS, а не на `where org_id = …`
> внутри функции; копия правил разошлась бы с политикой при первой правке RLS, и
> разошлась бы молча. **Следствие: объём выгрузки у `owner` и `manager` РАЗНЫЙ** —
> ровно на разницу их прав по RLS. Это свойство, не дефект; кнопка в UI показана только
> владельцу, чтобы у manager не возникло впечатления, что он выгрузил всю организацию.
...
> TS-списка валит `tests/unit/org-export.test.ts`. Имя таблицы в `format(%I)` берётся из
...
| created_at / updated_at | timestamptz | |
| settings | jsonb NOT NULL | **076 (R2-P0-D)**, DEFAULT `'{}'`. Настройки org. Известные ключи: `reconnect_days` int 3..90 (порог тишины, был хардкодом `RECONNECT_THRESHOLD_DAYS`), `stage_dwell_defaults` `{default,<phase_group>: int 1..365}` — **с S-R2-DWELL-CFG питает бейдж «залипла»** через `resolveDwellThreshold` (`deal-health.ts`), правится owner'ом в «Настройки организации»; **отсутствие ключа ≠ `null`**: пустое поле формы ключ НЕ пишет, иначе `??`-цепочка резолвера не дошла бы до фолбэка `STALE_BY_PHASE`. `completeness_rules` `{<ключ правила>: {weight: int 0..10}}` — **S-R3-TRUST-1**, веса правил полноты сделки поверх `DEFAULT_RULES` (`lib/domain/deal-completeness.ts`), `weight 0` = «правило не учитывать»; переопределяется **только вес** (label и «цена пустоты» — тексты продукта), неизвестный ключ при чтении игнорируется. В интерфейсе `OrgSettings` (`src/types/database.ts`) ключа НЕТ намеренно — файл руками не правится, чтение идёт через `readCompletenessOverrides()` поверх `.passthrough()`. `stage_target_days` `{<stage_id>: int 1..365}` — **S-PIPELINE-COCKPIT-1**, org-оверрайд нормы дней на КОНКРЕТНУЮ стадию для тайм-заливки ячейки кокпита (`resolveStageNorm`, `lib/domain/stage-norm.ts`); нет ключа/стадии ⇒ норма группы (`stage_dwell_defaults` → `STALE_BY_PHASE`), то есть заливка и бейдж «залипла» по построению меряют одну величину. Колонки `pipeline_stages.target_days` НЕТ и не будет: `pipelines`/`pipeline_stages` — глобальные словари вне тенант-модели (RLS `USING true`), развилка S-R2-DWELL-CFG закрыта. Читается `readStageTargetDays()` поверх `.passthrough()` (в `OrgSettings` ключа нет — файл руками не правится); правится owner'ом в «Настройки организации» → «Нормы стадий» (**S-STAGE-NORMS-UI-3**), пара `stageTargetsToFormValues`/`buildStageTargetDays`; пустое поле ключ НЕ пишет, очистка всех полей убирает ключ целиком. CHECK'а на схему значения **нет** (ключи растут по спринтам) — форму держит клиентский Zod `validators/org-settings.ts` |
...
- `tg_apply_capture(p_actor_id, p_draft_id, p_kind default null)` → jsonb _(DEFINER, только
  `service_role`; 110, **+124 ветка `task` — applied `20260819175811`**, **+125 привязка контакта к компании — applied `20260821104527`**)_ — применение черновика: создаёт
  ...
  **роль ≠ `viewer`** (42501) → статус → **черновик `task` нельзя применить чужой веткой**
  (22023, 124) → ветка. Возврат `{status}`: `created` (+ `id`, `kind`, `label`) |
  **`duplicate_inn`** (111; + `id`, `kind='company'`, `label`) | `already_applied` |
  `cancelled` | `not_found` | `kind_required`.
  ⚠️ **Ветка `task` (124) валидирует КАЖДУЮ ссылку из payload на принадлежность
  `draft.org_id`** — `assigned_to` через `memberships`, `project_id` через `projects`,
  `company_id` через `companies`. Не прошедшая проверку ссылка **обнуляется**, а не роняет
  вставку: черновик мог пережить удаление сотрудника, и терять из-за этого весь разбор
  незачем. ⚠️ **Ветка `contact` (125) тем же гардом проверяет `company_id`** и пишет привязку в
  `contact_company` (`role` = должность, `is_primary = true`), а не в колонку: `company_id`
  у `contacts` НЕТ. Вставка — под `exception when unique_violation then null`: контакт к
  этому моменту уже создан. Функция DEFINER читает и пишет в обход RLS — без этого гарда чужой uuid в payload
  дал бы cross-tenant связь (класс `check_task_dependency_valid`, 048).
  ⚠️ **`assigned_to` — единственное поле без отката.** `trg_notify_task_assigned` срабатывает
  AFTER INSERT: ошибочное назначение уведомляет не того человека немедленно. Ровно поэтому
  резолв упоминаний (`_shared/capture-resolve.ts`) отдаёт id **только при единственном
  совпадении**, а карточка в боте показывает все привязки ДО нажатия «Создать».
...
### companies _(002, +041 phones applied 2026-07-13, +102 юрреквизиты applied 2026-08-03, +103 okved applied 2026-08-03, +123 chz_groups applied 2026-08-12)_
...
| chz_groups | text[] | _(123 applied 2026-08-12)_ · **ПОДТВЕРЖДЁННЫЕ** товарные группы «Честного Знака» (названия из `src/lib/data/chz-groups.ts`, снапшот, не FK). NULL = не выяснено; `'{}'` = выяснено, что групп нет. ⚠️ **Не дубль `okved`, а вторая сущность:** `matchChzGroups(okved)` ВЫВОДИТ гипотезу из кода реестра, эта колонка хранит то, что подтвердил человек. Оптовик с ОКВЭД 46.x может возить обувь и молоко — реестр об этом не знает, продавец знает. Пишут двое: `convert_lead` v3 (перенос с лида, только в пустое) и `CompanyModal` (пикер, копия LeadModal). Кто побеждает при рендере — решает `resolveChzProfile` (`src/lib/domain/chz-profile.ts`): declared > derived, а имена, которых нет в снапшоте, идут в `unknown` и рендерятся нейтральным тегом (статуса у них нет, `ChzBadge` соврал бы про обязательность) |
...
**Realtime (122, applied 2026-08-10 · `20260810121513`):** `companies` добавляется в publication
`supabase_realtime`. До 122 её там не было, хотя `use-companies.ts:137` держит
`useRealtimeSync` с S-R2: канал открывался, событий не приходило никогда.
...
**Realtime (122, applied 2026-08-10 · `20260810121513`):** `contacts` добавляется в publication
`supabase_realtime`. До 122 её там не было, хотя `use-contacts.ts:151` держит
`useRealtimeSync` с S-R2: канал открывался, событий не приходило никогда.
`REPLICA IDENTITY` — default (PK).

### contact_company _(002, +093 cc_update applied 2026-08-02)_ — M:N контакт↔компания

⚠️ **ЭТО ЕДИНСТВЕННОЕ МЕСТО СВЯЗИ КОНТАКТА С КОМПАНИЕЙ.** Колонки `company_id` у `contacts`
нет и не было — код, который её ищет, написан по догадке. Писатели: UI карточки контакта
(`useLinkContactCompany`) и, с **125**, ветка `contact` в `tg_apply_capture`.
...
**`convert_lead` v3 (123, S-LEAD-CARRY-1 — applied 2026-08-12).** 119 переносила
историю; три поля квалификации при этом по-прежнему испарялись или оседали только
текстом. Три переноса поверх v2, тело взято из живой БД, сигнатура снова не меняется:

- **123-A `chz_groups` → `companies.chz_groups`.** Стоит сразу после блока «1. Компания»
  и **до** создания сделки: профиль компании не должен зависеть от того, чем кончится
  вставка сделки. `AND c.chz_groups is null` — **заполняем только пустое**; слияния
  профилей нет намеренно (автослияние массивов молча смешало бы профили двух разных
  лидов). Расхождение остаётся видимым человеку: `pinned_note` сделки всегда несёт
  строку «ЧЗ-группы: …» с тем, что сказал ЭТОТ лид. Пустой массив у лида не едет —
  `'{}'` это «выяснили, что групп нет» у ЛИДА, а не подтверждение по КОМПАНИИ.
- **123-B `decision_role` → строка в `deal_stakeholders`.** Создаётся **всегда**, даже
  при невыясненной роли: контакт, с которым вели лид, уже участник решения, а
  `role IS NULL` карта рендерит подписью «роль не указана»
  (`STAKEHOLDER_ROLE_EMPTY_LABEL`). Пустая карта у свежей сделки — потеря знания, а не
  чистота. `org_id` берётся **явно из строки лида**, не из `current_org_id()`: функция
  DEFINER и обязана работать в service-контексте, где `auth.uid()` = NULL и helper
  вернёт NULL (урок 024); `set_org_id()` явное значение переживает — он пишет только
  при NULL. `ON CONFLICT (project_id, contact_id) DO NOTHING` — ради идемпотентности
  повторного вызова (у свежей сделки конфликта быть не может).
- **123-C `budget_status` → строка `«Бюджет: …»` в `pinned_note`.** Ярлыки — зеркало
  `LEAD_BUDGET_STATUS_CONFIG` (`src/lib/validators/lead.ts`); дубль осознанный, того же
  класса, что зеркало `chz-groups` клиент↔edge — SQL не читает модули TS. `unknown` не
  пишется намеренно: «бюджет не выяснен» — отсутствие знания, а строка о нём в
  закреплённой заметке создаёт вид, что вопрос закрыт.
...
**Ролевой смок 123 (гейт, 2026-08-12, одна транзакция → `RAISE` для отката).** Второй
тенант и тестовые компания/контакт/лиды заводились под `postgres`, конверсия — под
`authenticated` с подменой `request.jwt.claims`. Результат:

| Проверка | Итог |
|---|---|
| `decision_role` → `deal_stakeholders` | строка создана, `role = champion`, `org_id` орг лида, контакт тот же |
| `pinned_note` | `Боль: … \| Бюджет: Подтверждён \| ЧЗ-группы: Обувь, Молочная продукция \| Дедлайн маркировки: 01.09.2026` |
| `chz_groups` → новая компания | `Обувь, Молочная продукция` |
| Профиль уже заполнен | НЕ перезаписан (`Табачная продукция` осталась) |
| Роль не выяснена | строка стейкхолдера всё равно создана, `role = NULL` |
| Бюджет `none` | в заметке `Бюджет: Нет бюджета`; `unknown` не пишется |
| `ON CONFLICT (project_id, contact_id)` | повторная вставка — 0 строк, роль не перезаписана |
| org-гард блока 1 | компания чужой орг с тем же владельцем отклонена |
| Чужак (случайный `sub`) | `42501 lead not found or access denied` |
| CHECK `leads_decision_role_check` | `'ceo'` → `23514` |

Advisors после apply — **идентичны базлайну** (4 INFO `rls_enabled_no_policy` на
`telegram_*`, 30 WARN 0029, 1 WARN `auth_leaked_password_protection`); в
performance-наборе ни одного упоминания `chz_groups`, `convert_lead` или
`decision_role`.

⚠️ **`regulatory_deadline` в компанию НЕ переезжает.** Дедлайн — свойство товарной
группы (справочник `chz-groups.ts`, поле `since`), а не компании: вторая копия даты
молча разошлась бы со справочником. У компании живут группы, дата выводится из них.
⚠️ **Бэкфилла конвертированных лидов НЕТ** — на 2026-08-11 их ноль
(`count(*) filter (where status='converted' and chz_groups is not null)` = 0).
Гранты не трогаются: `create or replace` сохраняет ACL.
...
**Realtime (122, applied 2026-08-10 · `20260810121513`):** `activity_log` добавляется в publication
`supabase_realtime`. До 122 её там не было, хотя `use-activity-log.ts:13` и
`use-entity-timeline.ts:212` подписаны на неё с 042 — обе подписки были мёртвыми, и
молчали тихо. ⚠️ **Эффект шире двух хуков:** таблица питает ленту сущности, org-ленту
дашборда и правый дровер — после apply они станут живыми у всех, кто держит страницу
открытой. Это цель, но писать `activity_log` умеют и триггеры аудита полей сделки (087)
с частотой выше пользовательских действий ⇒ на смоке отдельно смотреть частоту
перерисовки дровера; заметна — сужать `queryKey` инвалидации в `use-activity-log`, а не
откатывать миграцию. `REPLICA IDENTITY` — default (PK).
...
- **`p_entity_type` — ПЯТЬ значений (120): `org | project | company | contact | lead`.**
  **`lead` (120, applied 2026-08-10 · `20260810104424`).** Работают три ветки из
  шести: `src_calls`, `src_tasks`, `src_activity` (прямая ветвь). `src_meetings`,
  `src_projects`, `src_ai` предиката лида НЕ получают — колонки `lead_id` у
  `meetings`, `projects` и `ai_runs` нет (118 её там не заводила), и лишний конъюнкт
  упал бы `42703`. Это решение дизайна сущности: встреча до квалификации редка,
  транскрипта до звонка не бывает, а сделка из лида доступна прямой ссылкой
  «К сделке» на карточке. `create or replace` **без `drop`** — ни аргументы, ни набор
  колонок не меняются (в отличие от 113/114/115), гранты сохраняются.
...
- ⚠️ **Сигнатура заменялась ЧЕТЫРЕЖДЫ, и каждый раз через `drop`, а не перегрузкой.**
  ...
  повторены явно.
- **`parent_type` / `parent_id` (115, +`lead` в 120) — к чему относится событие.**
  Приоритет один на все ветки: `project` → `company` → `contact` → **`lead`
  (последним)**, первый непустой;
  ⚠️ **лид именно ПОСЛЕДНИЙ, и это поведение, а не порядок строк**: `convert_lead`
  (119) проставляет звонку/задаче `project_id`, а `lead_id` намеренно оставляет —
  поставь лид первым, и после конверсии org-лента показывала бы родителем лид вместо
  сделки, то есть ссылку в прошлое. Отсюда правило: `parent_type='lead'` возможен
  только у события НЕконвертированного лида; у `src_projects`
...
- **`public.export_org_data(p_org_id uuid) → jsonb`**. `language plpgsql`,
  **`security invoker`**, `set search_path = public`, `revoke all … from public, anon`
  → `grant execute to authenticated`.
- **Форма ответа** — `{ meta, members, data }`:
  `meta` = `{org_id, exported_at, exported_by, format: 'dashboard-crm/org-export@1', tables}`;
  `members` = массив `{profile_id, role}` (состав org отдельным ключом — `memberships`
  целиком, со своими `id`, для переноса бесполезна);
  `data` = `{ <таблица>: [строки] }` по 34 таблицам, пустая таблица даёт `[]`, не `null`.
- ⚠️ **`SECURITY INVOKER` — осознанное отклонение от конвенции** (третий случай после
  `entity_timeline`). Изоляцию организаций держит RLS, а не фильтр внутри функции:
  у `DEFINER` одна ошибка в `where org_id = …` выгрузила бы чужую org, а копия правил
  разошлась бы с политикой при первой же правке RLS — и разошлась бы молча. Экспорт по
  определению отдаёт то, что вызывающий и так вправе прочитать.
- ⚠️ **Объём выгрузки зависит от роли.** У `manager` файл меньше, чем у `owner`, ровно
  на разницу их прав по RLS — это свойство. Кнопка в UI (`OrgExportSection`) показана
  **только владельцу**: у manager она создавала бы впечатление полной выгрузки.
- ⚠️ **Явная проверка членства — не дубль RLS.** `memberships.profile_id = auth.uid()`
  (**колонки `user_id` в таблице нет**). Без неё посторонний получил бы валидный объект
  с 34 пустыми массивами вместо `42501`; сама проверка тоже идёт через RLS
  (`membership_select_own_org`), поэтому чужую org не подтвердит.
- ⚠️ **Имя таблицы в `format(%I)` берётся из литерала-массива в теле функции**, не из
  аргумента: пользовательский ввод в идентификатор не попадает ни при каком вызове;
  `p_org_id` уходит только параметром `$1`.
- **Состав — контракт, а не «все таблицы с `org_id`».** 34 из 50; исключены
  `webhook_endpoints`/`webhook_deliveries` (secret_id → Vault), четвёрка `telegram_*`
  (транспорт, очереди, одноразовые токены), `ai_runs` и `automation_runs` (журналы
  прогонов), `notifications`, `conversation_reads`, `invitations`, `memberships`,
  мёртвая `activities`, а также `project_files`/`project_videos`/`message_attachments`
  (метаданные без самих файлов вводят в заблуждение — S-EXPORT-2). Причина по каждой —
  `src/lib/domain/org-export.ts` → `EXCLUDED_TABLES`.
- **Лимит 50 000 строк на таблицу — предохранитель, не пагинация.** На 2026-08-21 самая
  крупная `activity_log` — 1357 строк. Порядок строк не задан (у `contact_company`,
  `conversation_members`, `message_reactions` нет одиночного `id`); при упоре в лимит
  срез был бы произвольным — допустимо ровно потому, что до лимита три порядка.
...
- ⚠️ **Это НЕ словарь ролей.** Словарь — CHECK `deal_stakeholders_role_chk` (092), шесть
  значений MEDDIC, он не меняется. Здесь ОЖИДАНИЕ ролей в конкретной воронке — другая
  сущность с похожим именем. CHECK на `role` — **дословное зеркало**: сослаться на чужой
  CHECK нельзя, а расхождение даёт 23514 уже на сиде. Меняются вместе, и вместе с
  `STAKEHOLDER_ROLES` / `STAKEHOLDER_ROLE_ORDER` / `STAKEHOLDER_ROLE_CONFIG` и
...
- **Сид (в миграции, идемпотентный, best-effort — паттерн 027):** дефолтная org
  ...
  `economic_buyer`. **Состав ERP — предположение**, спека описывает только IIoT;
  подтверждается владельцем.
