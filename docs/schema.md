...
> ⚠️ **Находка разведки, в 121 НЕ входит:** в publication нет `contacts`, `companies`
> и `activity_log`, хотя четыре хука на них подписаны — подписки мёртвые. Записано в
> `_analysis/BACKLOG.md`: включение `activity_log` оживит ленты во всём приложении и
> требует своего смока.
> Следующая свободная после неё — **122**;
