...
> Следующая свободная после неё — **114**;
...
### Лента сущности (112 applied `20260808204308` + 113 **applied 2026-08-08 `20260808222500`**) — сборка на сервере, страницами
...
- ⚠️ **113 ЗАМЕНИЛА сигнатуру, а не расширила.** Трёхаргументная версия 112 удалена
  первой строкой миграции: перегрузка дала бы PostgREST `PGRST203` (ambiguous) на любом
  вызове с `{p_entity_type, p_entity_id, p_limit}`. Гранты после `drop`+`create` не
  наследуются и повторены явно.
...
- **Долг, не закрытый в S-TL-2:** клиентский фильтр по `kind` фильтрует **загруженные
  страницы**, а не ленту — серверный фильтр `p_kinds text[]` идёт в S-TL-3. Там же
  `org_timeline()` для дайджеста. Шесть новых источников (`stage_transitions`, `messages`,
