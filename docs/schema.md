...
> **Applied:** вся цепочка 001–050 в проде (см. дельты 049/050 ниже). Открытый хвост
...
`notify` Workflow Engine → owner/creator). Пишутся только definer-триггерами
...
### automation_rules / automation_runs _(029, applied; обобщены в 050 S-WF-2A)_ — Workflow Engine
...
| trigger_type | text | **050 CHECK** `stage_entered\|status_changed\|field_changed` |
| trigger_config | jsonb | stage_entered `{"stage_id":uuid}`; status_changed `{"to":"won"\|null}`; field_changed `{"field":"budget"}` |
| action_type | text | **050 CHECK** `create_task\|notify\|create_activity\|set_field` |
...
| trigger_key | text | **050** NOT NULL — ключ идемпотентности, обобщённый со `stage_id` (stage_id::text для stage_entered; new.status / изменённое поле; COALESCE `'__null__'`) |
