...
>
> **Applied (в живой БД, ref `uoiavcabxgdjugzryrmj`):** миграции 001–048 — вся цепочка в проде (035–038 delivery, 039 reorder_tasks применены 2026-07-12; **040 rls_hardening + 041 multi_phone применены 2026-07-13**; **042 activity_log entity-links, 043 won_reason, 044/044b spawn owner, 045 notify_deal_won, 046 tasks Gantt-даты — Волна 2, применены/сверены по проду 2026-07-15**; **047 DROP legacy `projects.stage`/`deal_stage` — применено через MCP, файла миграции в репо нет; 048 task_dependencies (Gantt-зависимости) применены 2026-07-16**; verified через MCP list_migrations + интроспекция живой БД 2026-07-14). Фаза 1 multi-user
...
> **Pending:** **049 `task_dep_created_by_default`** (DEFAULT `auth.uid()` для `task_dependencies.created_by`) — файл в репо, ждёт гейта Cowork (apply_migration). Остальная цепочка 001–048 в проде. Открытый хвост
...
>
> **049 `task_dep_created_by_default` (S-DEPS-1 polish, PENDING — файл в репо, ждёт гейта Cowork):**
...
### notifications _(026, applied; +045 `deal_won`)_ — уведомления «тебе назначили»
v1: `task_assigned` (task.assigned_to) / `project_assigned` (project.owner_id);
_045_: `deal_won` (сделка выиграна → owner). Пишутся только definer-триггерами
...
| type | text | CHECK `task_assigned`/`project_assigned`/**`deal_won`** _(+`deal_won` в 045)_ |
...
### task_dependencies _(048, applied; +049 created_by default pending)_ — рёбра DAG между задачами (Gantt-зависимости, FS v1)
...
| created_by | uuid | → profiles ON DELETE SET NULL; **DEFAULT `auth.uid()` (049, pending)** |
