...
> **100 (S-CHAT-AUDIT-1) — НЕ ПРИМЕНЕНА, лежит на гейте** (ветка `feat/chat-audit-1`):
...
> (`use-messages.ts`: `.update({ body })` без `edited_at`) — между мержем и apply штамп
> не ставится вовсе; гейт применяет миграцию сразу. Откат — в шапке файла;
