...
> **086 (S-R2-DWELL-CFG) — ПРИМЕНЕНА `20260728160716`**; схемы не меняет — только сид-строка
> в `segments`;
> **087 (S-R2-FIELD-AUDIT) — ПРИМЕНЕНА `20260729063848`** (гейт Cowork 2026-07-29: advisors без
...
> **091 (S-R2-WEBHOOK-JOURNAL, эпик B2, финал) — ПРИМЕНЕНА `20260731065104`**: аддитивна —
...
> **093 (S-R2-TAIL-11-12, хвост #12) — ПРИМЕНЕНА `20260802113134`** (гейт Cowork 2026-08-02:
...
> **095 (снос legacy `project_messages`) — ПРИМЕНЕНА `20260802135816`** (гейт Cowork
> 2026-08-02, после мержа, деплоя и смока вкладки «Чат» на проде: новое сообщение ушло в
> `messages`, реакция поставилась через перевешенный FK, `conversation_reads` набил строки;
...
> **096 (S-CHAT-HUB-1c, эпик CHAT-HUB) — ПРИМЕНЕНА `20260802152152`** (гейт Cowork
> 2026-08-02: advisors ERROR нет, WARN 27→28 — ровно +1 `create_group_conversation` того же
> класса; **ре-смок по ВСЕМ типам каналов** после правки хелпера — регрессии нет (owner видит
> 19 каналов, manager 2, лента проекта на месте); смоки групп: создание через RPC (автор
> участник, чужие `p_member_ids` отброшены), чужак группы не видит, не-автор не переименовал
> и не удалил, выход своей строкой убирает канал из видимости, UPDATE/DELETE `general` и
> `project` не проходят ни у owner'а; следов смоков в проде нет): аддитивна —
...
> **097 (S-CHAT-HUB-1d, эпик CHAT-HUB) — ПРИМЕНЕНА `20260802155940`** (гейт Cowork
> 2026-08-02: WARN 28→29 — ровно +1 `can_access_chat_file`; главный смок пройден —
> участник группы видит оба объекта, менеджер вне группы видит только файл своего
> проекта, объект группы для него не существует; битый путь / traversal / пустой /
> NULL → `false`, не `22P02`; пустое тело сообщения проходит CHECK, вложение к чужому
> сообщению → 42501; `chat_files_delete` **правкой гейта** получил `owner = auth.uid()`
> плюс модерацию — иначе участник, не имеющий права снести чужое сообщение, стёр бы
> приложенный к нему файл. ⚠️ Прямой DELETE из `storage.objects` блокирует триггер
> Supabase `protect_delete()` — политика удаления проверяется только через Storage API,
> SQL-смоком НЕ покрыта): вложения
> в сообщениях. Приватный бакет **`chat-files`** (25 МБ; в проде до неё только `avatars` и
> `project-files` — сверено 2026-08-02), хелпер `can_access_chat_file(text)` и три политики
> `storage.objects` (`chat_files_select`/`_insert`/`_delete`). ⚠️ Модель доступа к бакету
> **отличается от `project-files` (055)**: там own-path (`foldername[1] = auth.uid()`), здесь
> путь начинается с КАНАЛА — `<conversation_id>/<message_id>/<uuid>.<ext>` — и доступ даёт
> `is_conversation_member()`, потому что вложение обязан видеть весь канал. Каст первого
> сегмента в uuid живёт в хелпере под перехватом исключения (битый путь = отказ, а не
> `22P02` на весь листинг). Плюс таблица `message_attachments` (RLS зеркалит видимость
> сообщения; вложение цепляется только к своему сообщению; realtime-публикация) и
> **ослабление `messages_body_check`** до `char_length(body) <= 4000` — сообщение может быть
...
> смока восстановлена; следов нет. **Гейт добавил седьмую таблицу — `activity_log`**,
> без неё таймлайн на карточках оставался бы личным): командная
...
> **100 (S-CHAT-AUDIT-1) — ПРИМЕНЕНА гейтом 2026-08-03, версия `20260803103435`**:
> `messages.edited_at` перестаёт быть клиентским полем. Триггерная функция
> `public.set_message_edited_at()` (DEFINER, `search_path = public, pg_temp`, EXECUTE
> только у `service_role` — клиент её не зовёт) + `trg_set_edited_at`
> **BEFORE UPDATE ON public.messages FOR EACH ROW**: `body` изменился → `now()`, иначе →
> `old.edited_at`. **Значение из payload клиента затирается в обеих ветках.** ⚠️ До 100
> штамп ставил хук (`useEditMessage`), а `messages_update` (094) сравнением старого с
> новым не занимается — автор одним запросом менял тело и слал `edited_at: null`, после
> чего пометка «· изм.» не появлялась и сообщение выглядело исходным; второй копии текста
> нет нигде (`activity_log` сообщения не пишет). Имя триггера **без префиксов `aa_`/`zz_`
> намеренно**: соседи (`trg_aa_freeze_org_id` — только `org_id` и только при его смене,
> `trg_set_org_id` — вообще на INSERT) с ним не пересекаются, порядок безразличен.
> Колонок не добавляет ⇒ **реген типов не нужен**. Клиентская часть едет тем же PR
> (`use-messages.ts`: `.update({ body })` без `edited_at`). Откат — в шапке файла.
> **Смок гейта (2026-08-03):** под автором сообщения, роль `authenticated`, две атаки
> в одной транзакции — `set body = body || ' x', edited_at = null` и
> `set body = body || '!', edited_at = '2020-01-01'` — **обе отбиты**: штамп встал в
> `now()`, не в NULL и не в 2020. Обратная ветка: `set body = body` (тело не менялось)
...
> **102 (S-INN-1) — ПРИМЕНЕНА гейтом 2026-08-03, версия `20260803131710`; edge `company-lookup` v1 задеплоен** (ветка `feat/inn-lookup`): юрреквизиты
...
> ложится чисто. Реген типов выполнен гейтом 2026-08-03: стаб
...
> ⚠️ **`okved` и `industry` — разные вещи, это инвариант спринта.** `okved` — факт реестра
> (код `10.13.2`), пишется только lookup-ом; `industry` — рабочая классификация для человека
> («Производство пищевых продуктов»), она рендерится в таблице компаний, в CommandPalette
> (`sub`) и уезжает в CSV-экспорт. Отрасль выводится из кода **локальным справочником**
> `src/lib/data/okved.ts` (разделы ОКВЭД-2 + переопределения по классу), а не хранится
> второй копией в БД: дублировать словарь в данных значит гарантированно разойтись с ним
> при первой же правке справочника. Код в поле «Отрасль» не пишется никогда — 5 записей с
...
> **104 (S-COMPANY-AI-1) — ПРИМЕНЕНА гейтом 2026-08-03, версия `20260803194153`; edge `ai-run` v9 задеплоена** (ветка `feat/company-ai` от `main` `243d1c1`):
...
> Ролевые смоки гейта (транзакции с rollback): член org создаёт `company_brief` по своей
> компании — проходит; несуществующий `entity_id` и подмена `entity_type` — 42501; чужой uid
> видит 0 прогонов. Advisors без изменений (30 WARN, тот же набор).
...
> `priority`, `telegram_notification_text()` приписывает к заголовку `task_reminder`
> « · важно» / « · критично». `normal`, отсутствующий ключ и мусор дают **пустую** приписку
> (`else ''`, не `else null` — конкатенация с NULL съела бы заголовок целиком).
> Тип возврата у обеих прежний ⇒ DROP не нужен, в отличие от `claim_telegram_outbox` в 108.
> ⚠️ **Редеплой edge НЕ нужен**: обе функции живут в БД, `telegram-send` получает готовый
> текст строкой очереди. Обратная совместимость полная — уведомления, созданные до 109,
> лежат без ключа `priority` и рендерятся ровно как раньше.
>
> **110 (S-TG-3) — applied 2026-08-08 `20260808174157`** (`110_telegram_capture.sql`; номер сверен запросом к
> `schema_migrations` 2026-08-08 — последняя применённая `20260808170347`). Быстрый ввод
> компании и контакта из пересланного боту текста. Новая таблица
> **`telegram_capture_drafts`** (черновик разбора: `id` уезжает в `callback_data`, куда поля
> не влезают — лимит 64 байта), две новые DEFINER-RPC **`tg_apply_capture(uuid, uuid, text)`**
> и **`tg_cancel_capture(uuid, uuid)`**, `cleanup_telegram_transport()` переписана
> (`create or replace`, тип возврата прежний ⇒ DROP не нужен). **Новых cron-джоб нет** —
> черновики чистит существующая `tg-cleanup`. **Требует редеплоя `telegram-webhook`**;
> apply раньше деплоя безопасен (таблица пустая, RPC никто не зовёт), обратный порядок —
> нет: новая функция звала бы несуществующую RPC на каждое сообщение.
> ⚠️ `kind` допускает **три** значения (`contact|company|unclear`), а не два: до нажатия
> кнопки ветка не выбрана, и записать её значением, которого человек не выбирал, значит
> нарушить инвариант «не гадаем за него». ⚠️ `duplicate_id` — **полиморфная ссылка**
> (companies ИЛИ contacts) ⇒ FK нет, как у `transcripts.entity_id`. ⚠️ Ретеншн черновиков —
> **сутки**, а не 30 дней как у остальных транспортных таблиц: там лежит сырой текст
> сообщений человека. ⚠️ RLS включена, политик нет — как `telegram_outbox`/`telegram_updates`,
> это транспорт, и клиенту он не нужен.
>
> **111 (S-TG-3-INN-DUP) — applied 2026-08-08 `20260808191356`** (`111_capture_inn_duplicate.sql`; номер
> сверен запросом к `schema_migrations` 2026-08-08 — последняя применённая `20260808174157`).
> Дубль по ИНН стал штатным исходом. Таблиц, индексов и cron **не трогает** — только
> `create or replace` у `tg_apply_capture` (сигнатура и тип возврата прежние ⇒ DROP не нужен).
> Вставка компании обёрнута блоком `exception when unique_violation`: функция находит
> существующую запись по `(org_id, inn)`, **закрывает черновик как `applied`** и возвращает
> новый исход **`duplicate_inn`** с `id`/`label`. ⚠️ **Было:** вставка упиралась в
> `uq_companies_org_inn`, исключение откатывало функцию вместе с `status='applied'` — человек
> видел «Не удалось, откройте CRM» без единого слова про ИНН, а кнопка «Всё равно создать»
> оставалась живой и давала ровно тот же ответ. ⚠️ Обработчик стоит **только на вставке
> компании**, а не вокруг тела: `when unique_violation` на функции целиком поймал бы будущие
> констрейнты и соврал бы про причину. Ветка контакта не тронута — на `contacts` уникальных
> индексов нет (сверено 2026-08-08). ⚠️ `id` в ответе может быть **NULL** (запись удалили
> между падением вставки и поиском) — бот в этом случае шлёт текст без ссылки.
...
>
> **112 (S-TL-1, ось 2 «Единая лента событий») — applied 2026-08-08 `20260808204308`**
...
> отклонение осознанное, см. раздел «Лента сущности» ниже. Применена гейтом Cowork
> 2026-08-08, версия из `schema_migrations` — `20260808204308`.
>
> **113 (S-TL-2, ось 2 «Единая лента событий») — applied 2026-08-08 `20260808222500`**
> (`113_entity_timeline_keyset.sql`; номер сверен запросом к `schema_migrations`
> 2026-08-09 — последняя применённая `20260808204308`). У ленты появляется дно:
> **лимит переезжает с источника на страницу** + keyset-курсор `(ts, id)`.
> ⚠️ **Сигнатура `entity_timeline` ЗАМЕНЕНА, а не расширена**: первой строкой миграции
> идёт `drop function public.entity_timeline(text, uuid, int)`, следом создаётся
> `entity_timeline(text, uuid, timestamptz, text, int)`. Перегрузка недопустима —
> PostgREST при двух кандидатах с пересекающимися именами аргументов отвечает
> **`PGRST203` (ambiguous)**, и вызов `{p_entity_type, p_entity_id, p_limit}` подошёл бы
> обоим. Клиент обновляется тем же PR, окна несовместимости нет. Таблиц, колонок, enum
> и cron не трогает ⇒ **реген типов нужен только ради сигнатуры функции**; набор
> возвращаемых колонок прежний. Редеплоя edge не требует. Плюс **3 индекса по выражению**
> `(entity_id, coalesce(deadline, created_at) desc)` на `tasks` — под новую ось отбора;
> индексы 112 `idx_tasks_*_created` **не удаляются** (по ним ходит доска задач).
> **114 (S-TL-3, ось 2 «Единая лента событий») — applied 2026-08-09 `20260809083732`**
> (`114_entity_timeline_kinds.sql`; номер сверен запросом к `schema_migrations`
> 2026-08-09 — последняя применённая `20260808222500`). Фильтр по видам переезжает
> на сервер: **`p_kinds text[]`** со словарём из семи значений (шесть `kind` +
> производный `note`). ⚠️ **Сигнатура `entity_timeline` ЗАМЕНЕНА В ТРЕТИЙ РАЗ**:
> первой строкой `drop function public.entity_timeline(text, uuid, timestamptz, text, int)`,
> следом шестиаргументная — перегрузка дала бы PostgREST **`PGRST203`** (вызов с пятью
> аргументами подошёл бы обеим). Гранты после `drop`+`create` повторены явно.
> ⚠️ Вместе с фильтром **точная отсечка курсора переехала ВНУТРЬ каждой ветки union** —
> без неё сужение до одного источника обрывает пагинацию на живом хвосте (см. раздел
> «Лента сущности»). Таблиц, колонок, enum, индексов и cron не трогает ⇒ **реген типов
> нужен только ради сигнатуры функции**; набор возвращаемых колонок прежний. Редеплоя
> edge не требует.
>
> **115 (S-TL-4, ось 2 «Единая лента событий») — applied 2026-08-09 `20260809092051`**
> (`115_org_timeline.sql`; номер сверен запросом к `schema_migrations` 2026-08-09 —
> последняя применённая `20260809083732`). Та же функция обслуживает **уровень
> организации**: четвёртое значение `p_entity_type = 'org'`, при котором предикат
> сущности снимается целиком, а `p_entity_id` игнорируется (и потому получает
> `default null`). ⚠️ **Границу org держит RLS источников, а не тело функции** — ни
> `current_org_id()`, ни ручного `org_id = …` в org-ветви нет, см. раздел «Лента
> сущности». Контракт расширен двумя колонками — **`parent_type` / `parent_id`**
> (к чему относится событие; имя резолвит клиент из своих кэшей). Плюс два
> производных вида в `p_kinds` — `stage` и `deleted`. ⚠️ **`drop function` первой
> строкой — здесь НЕ из-за `PGRST203`**: список типов аргументов не меняется, но
...
>
> **117–119 (S-LEAD-CORE-1) — НАПИСАНЫ, НЕ ПРИМЕНЕНЫ** (номера сверены запросом к
> `schema_migrations` 2026-08-10 — последняя применённая по-прежнему `20260809092051`;
> файл 116 тоже ждёт гейта). Применяются СТРОГО по порядку: 118 опирается на колонки
> из 117, 119 — на обе. **117 `117_lead_work_fields.sql`** — двенадцать колонок работы
...
> та же ⇒ `create or replace` без `drop`. ⚠️ **Реген типов нужен** (новые колонки у
> четырёх таблиц): до него в `use-leads.ts` живут касты payload'ов, в `use-calls.ts`
> — три `as unknown as Call`. Редеплоя edge не требует.
...
>
> **047 DROP legacy `projects.stage` (S-LEGACY-STAGE-1 B2, applied через MCP 2026-07-16 — файла миграции в репо НЕТ):**
> Снесены `projects.stage` (индекс `idx_projects_stage` каскадом), тип `deal_stage`,
> триггеры `on_stage_change`+`log_stage_change` и `trg_ab_null_internal_stage`+`null_internal_stage`.
> `stage_id` (pipeline_stages) — единственный источник истины по стадии. **Backlog:** файла
> `047_*.sql` нет (применялось MCP `apply_migration`) — при желании восстановить реконструкцией
> из живого DDL; `stage_change`-события больше НЕ пишутся, историческая лента рендерит их через
> `LEGACY_STAGE_LABELS` (не удалять маппинг).
>
> **048 `task_dependencies` (S-DEPS-1, applied 2026-07-16):** рёбра DAG между задачами
> (Gantt-зависимости, FS v1) + валидатор `check_task_dependency_valid()` (DAG-инвариант держит
> триггер: граф ацикличен by construction, ребро замыкающее цикл → `P0001`). Таблица — см. раздел
> «task_dependencies» ниже.
>
> **049 `task_dep_created_by_default` (S-DEPS-1 polish, applied 2026-07-16):**
> `alter column task_dependencies.created_by set default auth.uid()` — аудит «кто связал» без правки
> клиентского хука (insert без `created_by` получит автора из DEFAULT; под service/MCP → NULL, ок).
>
> **050 `workflow_engine` (S-WF-2A, applied 2026-07-16):** обобщение S29-движка (триггеры/действия/условия).
...
> — **`wf_eval_conditions(jsonb,jsonb)`** — IMMUTABLE, non-DEFINER, fail-closed (неизвестный op / битый каст → не матчим);
...
> `next_action_date`/`probability` (**НИКОГДА** `stage_id`/`status`/`type`/`org_id`). Обобщение — см. раздел
> «automation_rules / automation_runs» ниже.
>
> **051 `task_overdue` (S-WF-2C-A, applied):** time-based триггер просрочки задач (pg_cron, НЕ row-триггер).
> — **`automation_rules_trigger_type_check`** += `'task_overdue'` (`action_type` CHECK не трогали — движок 051 умеет
> только `notify`/`create_activity`, ограничение на стороне UI-валидатора). — **`automation_runs.project_id` → nullable**
> (просроченные задачи бывают персональными, `project_id` NULL) + **partial-unique `automation_runs_rule_key_null_project_uniq`**
> `(rule_id, trigger_key) WHERE project_id IS NULL` (3-колоночный unique не дедупит NULL-строки — PG NULL≠NULL).
> — **`idx_tasks_overdue`** `(org_id, deadline) WHERE deadline IS NOT NULL AND lane <> 'done'` (partial-idx под ежедневный скан).
> — **`run_overdue_automations()`** (SECURITY DEFINER, revoke public/anon/authenticated, grant service_role): скан
> просроченных задач → `task_overdue`-правила org → идемпотентность `NOT EXISTS(rule_id, trigger_key=task_id::text)`
> (task_id глобально уникален — project_id в ключе не нужен) → `wf_eval_conditions(conditions, to_jsonb(task))` (условия
> по полям ЗАДАЧИ) → notify исполнителю (`COALESCE(assigned_to, created_by)`, `type='automation'`, `entity_type='tasks'`) /
> create_activity → `activity_log 'automation_fired'`; per-(rule,task) subtxn изоляция + внешний EXCEPTION-swallow.
> `create_task`/`set_field` для task_overdue — no-op (отсечены ДО записи run). Напоминает **один раз** на задачу (run уже есть).
> — **pg_cron** (`create extension if not exists pg_cron`) + job **`wf-overdue-daily`** `'0 6 * * *'` (06:00 UTC = 09:00 MSK).
>
...
> self-delete (`profile_id = auth.uid()`). NULL-safe deny; `protect_last_owner` оставлен
> нижней границей. **Смок гейта:** admin удалить/разжаловать owner → `42501`; admin →
> manager/viewer ок; admin поднять manager→admin → отказ WITH CHECK. Только RLS — код
> приложения (TeamSection UI-гейт owner-строки, роль-гейт create) не требует apply.

> **061 `onboarding` (Sprint S-ONBOARD-1 + T1c) — APPLIED 2026-07-18 (гейт Cowork; смок ролями + advisors без новых WARN):** одна миграция, 6 секций.
> - **[T1c] `accept_invitation` email-guard:** при несовпадении `lower(auth.email) <> lower(invite.email)` → статус `wrong_email` + `invited_email`, **accepted_at НЕ стемпится** (токен валиден для адресата). Чинит инцидент 18.07 (owner в своей сессии сжигал чужой инвайт). Включает полное тело 058 (порядок apply 058→061).
> - **profiles += `phone`/`job_title`/`onboarded_at`** (nullable) + backfill `onboarded_at=now()` для уже-именованных (owner на /welcome не попадёт).
...
| full_name | text | NOT NULL DEFAULT '' (заполняется на онбординге, 061) |
| avatar_url | text | публичный бакет `avatars`, путь `{uid}/…` (061) |
| phone | text | **061** — nullable, вводит сам юзер на `/welcome` |
| job_title | text | **061** — должность (текст, НЕ роль; роль в `memberships`) |
| onboarded_at | timestamptz | **061** — null = онбординг не пройден (welcome-гейт в middleware через `session_gate`) |
| ~~role~~ | text | **удалена в 024** _(applied S25)_ — роли только в `memberships` |
| settings | jsonb | DEFAULT `{}` |
| created_at / updated_at | timestamptz | |

> **Профиль self-service (061, S-ONBOARD-1):** `handle_new_user` берёт имя из `raw_user_meta_data->>'full_name'`, а magic-link/OTP его не несёт → у приглашённых `full_name=''`. Модуль онбординга: RPC `complete_onboarding(full_name,phone,job_title)` (DEFINER, гард «имя обязательно» → 23514, own-row update), RPC `session_gate()` (org_id + onboarded одним round-trip для middleware), бакет `avatars` (public read, запись только своя папка `{uid}/…`). RLS `profiles_update_own` (USING+CHECK `id=auth.uid()`) не менялась — аватар/правка в Настройках идут напрямую под RLS. Owner чужие профили видит (read), не редактирует. Форма `/welcome` (обязательна при `onboarded_at IS NULL`) + редактируемая карточка в Настройках — общий `ProfileForm`.
...
| payload | jsonb | DEFAULT `{}` (`{title}` — text задачи / name сделки). У **`task_reminder`** (108) состав шире: `title`, `text` (готовая строка ««задача» — срок ДД.ММ ЧЧ:ММ МСК · проект»), `deadline`, `project_name`, **`priority`** _(109 — `normal`/`important`/`critical` строкой; уведомления, созданные ДО 109, лежат без этого ключа, и это штатно)_ |
...
`trg_set_org_id` НЕ вешается (паттерн stage_requirements). **v1 (029) был жёстким**:
один `trigger_type='stage_entered'`, один `action_type='create_task'`.
**050 (S-WF-2A) обобщил движок** до 3 триггеров, 4 действий и AND-условий (`conditions`)
— обратно совместимо (seed-правила S29 остаются валидны под новыми CHECK).
...
| — | — | **051 partial-unique `automation_runs_rule_key_null_project_uniq`** `(rule_id, trigger_key) WHERE project_id IS NULL` — дедуп персональных task_overdue (PG NULL≠NULL обходит 3-колоночный unique) |

**Идемпотентность (050 обобщение v1)**: `UNIQUE(rule_id, project_id, trigger_key)`
= правило стреляет **один раз на пару (сделка, trigger_key)**. Для `stage_entered`
`trigger_key = stage_id::text` (обратно совместимо с backfill); для остальных
триггеров — новый статус / изменённое поле. Защита от спама при пинг-понге.
...
`tasks WHERE deadline < now() AND lane <> 'done'` (partial-idx `idx_tasks_overdue`) →
`task_overdue`-правила org → идемпотентность `NOT EXISTS(rule_id, trigger_key=task_id::text)`
(task_id глобально уникален, project_id в ключе не нужен → работает и для персональных задач) →
`wf_eval_conditions(conditions, to_jsonb(task))` (условия по полям **задачи**: priority/lane/text) →
`notify` исполнителю (`COALESCE(assigned_to, created_by)`, `type='automation'`, `entity_type='tasks'`) /
`create_activity` (`type='note'`, в timeline компании/контакта) → `activity_log 'automation_fired'`.
`create_task`/`set_field` для task_overdue — **no-op** (отсечены ДО записи run — иначе no-op run
навсегда заблокировал бы будущее расширение). **Напоминает один раз на задачу** (run уже есть →
повторная просрочка после reopen/сдвига дедлайна не стреляет). ACL: revoke public/anon/authenticated,
grant `service_role`.
...
**110 добавила в ту же функцию третью ветку** — `telegram_capture_drafts` старше **суток**
(см. ниже); новой cron-джобы под это НЕ заводилось. ⚠️ Правило «pending не трогаем» на
черновики **не распространяется**: там `pending` через сутки значит «человек не нажал
кнопку», а не поломку.

**`telegram_capture_drafts`** _(110, S-TG-3 — applied `20260808174157`)_ — черновик разбора между
...
черновика.
`id`, `org_id`, `profile_id` _(кто прислал — он же единственный, кто вправе подтвердить)_,
`kind` _(**contact | company | unclear** — три значения; до нажатия ветка не выбрана, и
писать её значением, которого человек не выбирал, значит гадать за него)_, `payload jsonb`
_(**обе** ветки `{contact, company}`, иначе на `unclear` вторая кнопка применяла бы пустоту;
реквизиты ЕГРЮЛ уже влиты в `company`)_, `source_text` _(разбор инцидентов «бот понял не
то»)_, `duplicate_id` + `duplicate_kind`, `status` _(pending | applied | cancelled)_,
`created_at`, `updated_at` _(триггер `public.update_updated_at()`)_, `applied_at`,
`created_entity_id`.
⚠️ **FK у `duplicate_id` нет** — ссылка полиморфная (companies ИЛИ contacts), тот же случай,
что `transcripts.entity_id`.
⚠️ **RLS включена, политик нет, гранты сняты — это законченное состояние.** Таблица
транспортная, как `telegram_outbox`/`telegram_updates`: пишет и читает её только edge под
`service_role`, а политика «на всякий случай» открыла бы браузеру чужой сырой текст сообщений.
⚠️ Заморозка `org_id` — **триггером руками** (`trg_aa_freeze_org_id`): автоцикл 054 покрыл
только таблицы, бывшие на момент 054.
...
- `telegram_notification_text(type, entity_type, entity_id, payload, app_url)` → text
  ...
  **109:** заголовок `task_reminder` получает приписку приоритета — `important` → « · важно»,
  `critical` → « · критично», `normal`/нет ключа/мусор → **ничего**. ⚠️ В ветке `else ''`,
  **не** `else null`: конкатенация с NULL дала бы NULL и съела заголовок целиком (та же
  ловушка, из-за которой вокруг `v_link` стоит `coalesce`). Приоритет **не экранируется** —
  литерал из enum, а не введённый человеком текст.
...
- `enqueue_task_reminders()` → void _(DEFINER, `service_role`)_ — планировщик напоминаний,
  ...
  **109:** в payload добавлен ключ `priority` (`tasks.priority` строкой). Решение «какой
  маркер показать» принимает `telegram_notification_text`, а не планировщик — иначе правка
  формата требовала бы двух миграций вместо одной.
...
- `tg_apply_capture(p_actor_id, p_draft_id, p_kind default null)` → jsonb _(DEFINER, только
  `service_role`; 110)_ — применение черновика: создаёт контакт или компанию **под правами
  актора**. Порядок гардов, все NULL-safe: аргументы (22023) → `p_kind` из
  `{contact,company}` или NULL (22023) → `select … for update` черновика → `profile_id =
  актор` (42501) → членство в org черновика (42501) → **роль ≠ `viewer`** (42501) → статус →
  ветка. Возврат `{status}`: `created` (+ `id`, `kind`, `label`) | **`duplicate_inn`** (111;
  + `id`, `kind='company'`, `label`) | `already_applied` | `cancelled` | `not_found` |
  `kind_required`.
  ⚠️ **`duplicate_inn` (111) — штатный исход, а не сбой.** Вставка компании обёрнута
  `exception when unique_violation` (только вставка, не всё тело): единственный уникальный
  индекс — `uq_companies_org_inn (org_id, inn) where inn is not null and inn <> ''`, поэтому
  функция ищет существующую запись и отдаёт ссылку на неё. Черновик при этом закрывается как
  `applied` — повтор бессмыслен, он упрётся в тот же индекс (единственное исключение из
  правила «упала вставка ⇒ остаётся `pending`»). `id` может быть **NULL**, если запись
  удалили между падением вставки и поиском.
  ⚠️ **Роль читается запросом к `memberships`, а не `current_org_role()`**: хелпер ходит в
  `auth.uid()`, который под service_role NULL, — гард пропускал бы всех.
  ⚠️ **`for update` обязателен**: два быстрых нажатия одной кнопки приходят двумя апдейтами
  и обрабатываются двумя isolate'ами; без блокировки оба увидели бы `pending` и создали бы
  две записи из одного текста — то есть дедуп обошёл бы сам себя.
  ⚠️ **Поля перекладываются поимённо, без `jsonb_populate_record`**: `payload` пишет edge,
  то есть строка приходит из-за периметра БД, а популятор дал бы ей право задать любую
  колонку — включая `org_id`/`owner_id`/`created_by`, которые ставит только функция.
  ⚠️ Порядок «вставка → отметка `applied`»: упадёт вставка — статус остаётся `pending` и
  повторное нажатие сработает.
- `tg_cancel_capture(p_actor_id, p_draft_id)` → jsonb _(DEFINER, только `service_role`; 110)_ —
  `cancelled | already_applied | not_found`. Отдельной RPC, а не UPDATE из edge: правило
  «чужой черновик не трогают» одно для обеих кнопок. Применённый черновик **не отменяется** —
...
- **`telegram-webhook`** — `/start <token>` (107), **`callback_query` (108)** и **свободный
  текст (110, S-TG-3)**.
  **ОБЯЗАТЕЛЬНАЯ** проверка
  ...
  ⚠️ **Порядок веток обязателен:** секрет → идемпотентность → `callback_query` → команды →
  свободный текст. Текст разбирается ПОСЛЕДНИМ; выше — он проглотил бы `/start`, то есть
  отправил бы токен привязки в модель.
  ...
  **Ветка свободного текста (110, S-TG-3; код — `telegram-webhook/capture.ts`).** Шаги:
  резолв привязки по `from.id` _(**org берётся оттуда же** — `telegram_accounts.org_id`;
  второго источника у бота нет, `current_org_id()` под service_role вернул бы NULL)_ →
  лимит **2000 символов** _(зеркало `CAPTURE_MAX_CHARS`)_ → «Разбираю…» **до** обращения к
  модели _(молчание бота человек читает как поломку и присылает текст повторно — то есть
  платит вторым прогоном модели)_ → `ai-capture` → `extractInn` + `company-lookup` → дедуп →
  черновик → `editMessageText` карточкой.
  ⚠️ **Это второй клиент готового кода, а не вторая его реализация.** Разбор, реквизиты,
  распознавание ИНН чексуммой и дедуп — общие с веб-виджетом модули в
  `supabase/functions/_shared/`. Собственной логики разбора в боте нет.
  ⚠️ **`ai-capture` и `company-lookup` объявлены `verify_jwt = true`, и ослаблять это
  нельзя** — обе открыты из браузера. Проверено прямым запросом к шлюзу (S-TG-3): шлюз
  требует валидный **проектный** JWT, а не пользовательскую сессию, — с ключом проекта
  возвращается **400** от тела функции, без ключа и с мусорным — **401**. Ключ `service_role`
  того же класса ⇒ `functions.invoke` от service-клиента проходит, обходной `fetch` не нужен.
  ⚠️ **`callback_data` — пять префиксов, и двоеточие входит в каждый:** `tgdone:` (108),
  `tgcap:`, `tgcapx:`, `tgcapc:`, `tgcapo:`. Без двоеточия `tgcap` жадно матчил бы три
  остальных, и «Отмена» выполнилась бы как «Создать». Разборы **раздельные**: `tgdone:` —
  про задачи, `tgcap*:` — про черновики.
  ⚠️ **Дубль по ИНН (111, S-TG-3-INN-DUP).** Карточка дубля, найденного **по ИНН**, не
  показывает «Всё равно создать»: вставка упрётся в `uq_companies_org_inn`, то есть кнопка не
  может сработать никогда. По **названию** кнопка остаётся — два юрлица с похожими названиями
  и разными ИНН обычны. Признак даёт `findCaptureDuplicate` полем **`matchedBy`**
  (`inn|name|email|phone`). Исход `duplicate_inn` из RPC бот показывает отдельным текстом
  «Компания с этим ИНН уже заведена: …» + url-кнопка на существующую карточку (без `id` —
  текст без кнопки: ссылка в никуда хуже её отсутствия).
  ⚠️ **Текст ошибки не предлагает действие, которое не может помочь.** `ai-capture` не
  ответила (сбой, лимит, нет ключа) или не записался черновик ⇒ «Разбор временно недоступен —
  это на нашей стороне»; «попробуйте ещё раз» остаётся только там, где ответ ПРИШЁЛ, но не лёг
  в схему. Признак различения — `invokeJson` возвращает `null` только на сбое вызова.
  ⚠️ **`parse_mode` у `editMessageText` — ДВА РАЗНЫХ СЛУЧАЯ.** Карточка подтверждения
  собрана нами и экранирована ⇒ уходит **с** `parse_mode: HTML`. Правка, пересылающая
  обратно текст, полученный от Telegram (S-TG-2), — **без** него. Запрет из 108 относится
  ко второму случаю, а не к разметке вообще.
  ⚠️ Дедуп читает списки org с потолком **2000 строк** _(на 2026-08-08 в проде 89 контактов
  и 268 компаний)_ — **долг**: после потолка дубль перестанет находиться молча.
...
| inn / industry / website / phone / address / notes | text | `phone` — legacy primary-зеркало массива `phones`; `address` — **фактический** адрес, юрадрес живёт отдельно _(102)_; `industry` — **человеческая** отрасль (таблица, CommandPalette, CSV), код ОКВЭД в неё не пишется _(103)_ |
...
| kpp / ogrn | text | _(102 applied 2026-08-03)_ · из ЕГРЮЛ через edge `company-lookup` |
| legal_name | text | _(102 applied 2026-08-03)_ · полное юрназвание с ОПФ. Отдельно от `name` — инвариант фичи |
...
**Индексы _(102 applied 2026-08-03)_:** `uq_companies_org_inn` — partial unique
...
### contact_company _(002, +093 cc_update applied 2026-08-02)_ — M:N контакт↔компания
...
**RLS (4 политики, 093):** `cc_select` — org; `cc_insert` — owner/admin/manager;
`cc_update` _(093)_ — owner/admin/manager, `WITH CHECK` повторяет `USING`;
`cc_delete` — **только owner/admin** (уже, чем insert/update — историческая асимметрия 002,
осознанно не тронута в 093).

⚠️ **До 093 UPDATE-политики не было вовсе** — роль контакта в компании нельзя было
отредактировать: под RLS это не ошибка доступа, а тихий 0 изменённых строк. Найдено разведкой
S-R2-D3 при сверке с новой `deal_stakeholders`. UI редактирования роли **пока нет** — 093
снимает блокиратор, интерфейс отдельным решением.

ℹ️ Три старые политики объявлены без `TO`-роли (`{public}`), `cc_update` — `to authenticated`,
как во всех миграциях с 082. Расхождение косметическое: `anon` грантов на таблицу не имеет.

**Смоки гейта 093 (прод, подопытная строка восстановлена, `SMOKE-%` — 0):** owner UPDATE — 1
строка; manager UPDATE — 1 строка; чужак — 0 строк; viewer — 0 строк.
**Tamper `org_id` под manager исключения НЕ даёт** — и это не дыра: `freeze_org_id()` не
`RAISE`, а **восстанавливает** (`new.org_id := old.org_id`), поэтому `WITH CHECK` видит уже
исправленное значение. Проверено фактом: `org_id` строки после смока прежний, строк в чужих
org — 0. Отличие от `deal_stakeholders` (там tamper дал 42501) в том, что там это был INSERT,
где OLD-строки нет и триггер не участвует.
...
| ~~stage~~ | ~~deal_stage enum~~ | **DROPped в 047 (S-LEGACY-STAGE-1 B2, applied через MCP 2026-07-16).** Колонка `projects.stage`, тип `deal_stage`, индекс `idx_projects_stage`, триггеры `on_stage_change`/`log_stage_change` и `trg_ab_null_internal_stage`/`null_internal_stage` — сняты. `stage_id → pipeline_stages` — единственный источник истины по стадии. Историческая лента `stage_change` рендерится через `LEGACY_STAGE_LABELS` (маппинг не удалять). |
| budget | bigint | |
| deadline | date | |
| next_step | text | |
...
| won_reason | text | _043_ причина выигрыша сделки (зеркало loss_reason; заполняется двухшаговой «Выиграна») |
...
| lead_id | uuid | _118 (**написана, не применена**)_ → leads ON DELETE SET NULL. Задача по лиду до конверсии; partial idx `idx_tasks_lead`. `convert_lead` (119) достраивает project/company/contact, `lead_id` не зануляет |
...
`current_org_id()`; гард только для auth-контекста `auth.uid() IS NOT NULL`, service/MCP не ломаем);
...
### calls _(005, +028 ai_summary, +118 lead_id **НАПИСАНА, НЕ ПРИМЕНЕНА**)_
...
### leads _(016, +018 dedupe, +117 поля работы **НАПИСАНА, НЕ ПРИМЕНЕНА**)_
...
### project_videos _(066, S-VIDEO-EMBED-1)_ — видео-материалы проекта (embed YouTube/VK/Rutube)
...
| provider | text NOT NULL | `CHECK in ('youtube','vk','rutube','other')` — только для badge; embed на рендере из `parseVideoUrl(url)` (stored provider НЕ доверяем — anti-XSS) |
| title | text | |
| sort_order | integer NOT NULL | DEFAULT 0 |
| created_by | uuid | → profiles ON DELETE SET NULL, DEFAULT `auth.uid()` |
| created_at | timestamptz | NOT NULL DEFAULT now() |

Индексы: `(project_id, sort_order)`, `(org_id)`, `(created_by)`.
Триггеры: `trg_set_org_id` (before insert).
**RLS (066):** `project_videos_select` — **зеркало `projects_select` + `projects_select_member`** (кто видит проект — видит видео): `org AND (current_org_role() in ('owner','admin') OR project ownership (owner_id/created_by) OR is_project_member(project_id))`; **БЕЗ manager** (иначе видео виднее самого проекта). `project_videos_insert/delete` — canManage: `org AND (owner/admin OR project ownership вкл. `created_by`)`. **NO UPDATE.** GRANT authenticated / REVOKE anon. **Hard delete.** Одна SELECT-политика (не две — нет multiple-permissive). Гейт-смок verified: владелец-не-member SELECT видит; member INSERT в чужой проект → 42501.
...
| org_id | uuid NOT NULL | → organizations ON DELETE CASCADE; ставит `trg_set_org_id` |
| project_id | uuid NOT NULL | → projects ON DELETE CASCADE |
| author_id | uuid | **nullable** → profiles ON DELETE SET NULL, DEFAULT `auth.uid()` (история чата переживает ушедшего автора) |
| body | text NOT NULL | `CHECK length(body) between 1 and 4000`; рендерится как текст (`whitespace-pre-wrap`, без `dangerouslySetInnerHTML` — XSS-контур) |
| edited_at | timestamptz | проставляется при правке |
| created_at | timestamptz | NOT NULL DEFAULT now() |

Индексы: `(project_id, created_at)`, `(org_id)`, `(author_id)`.
Триггеры: `trg_set_org_id` (before insert).
...
| lead_id | uuid | _118 (**написана, не применена**)_ → leads ON DELETE CASCADE. Partial idx `idx_activity_log_lead` WHERE NOT NULL. Пишет `log_lead_status_change()` (событие `lead_status_changed`). ⚠️ Журнал УДАЛЕНИЯ лида `lead_id` не заполняет — CASCADE снёс бы запись |
...
| Тип | Значения |
|-----|----------|
| ~~`deal_stage`~~ | **DROPped в 047** (тип и колонка `projects.stage` сняты; стадия — только `stage_id → pipeline_stages`). Ранее: new_lead, qualification, waiting_materials, preparing_kp, kp_sent, kp_review, preparing_docs, cz_approval, trilateral_meeting, experiment_setup, contract_review, contract_signing, won, lost |
...
- **Гард «последний owner»** `protect_last_owner()` — `BEFORE UPDATE OR DELETE`
  ...
  `RAISE EXCEPTION ... ERRCODE '42501'`. Нижняя граница; разжалование/удаление
  owner админом ловит 059 (protect_last_owner спасал лишь последнего).
- **059 (T2) — admin не трогает owner.** `membership_update` пересобрана с
  **USING (по старой роли) И WITH CHECK (по новой)**: owner — кого угодно; admin —
  только `role IN ('manager','viewer')` в USING (кого трогает) И в WITH CHECK (какую
  роль выставляет) → admin не поднимет manager→admin/owner (grok W1: одного USING
  мало — эскалацию `SET role` ловит только WITH CHECK). `membership_delete` — тот же
  предикат в USING (+ self-delete `profile_id = auth.uid()` сохранён). NULL-safe:
  `current_org_role() IS NULL` → deny.
...
- **`spawn_delivery_project` v3** _044_ / 044b (Win Wizard, applied 2026-07-15) — 4-арг
  сигнатура `spawn_delivery_project(p_deal_id uuid, p_kind text, p_template_id uuid
  DEFAULT NULL, p_owner_id uuid DEFAULT NULL) → uuid`. Owner delivery-проекта =
  `COALESCE(p_owner_id, deal.owner_id, auth.uid())`; назначаемый `p_owner_id`
  **валидируется как член той же org** (`memberships`) → иначе `42501`. Остальные
  гарды/тело v1–v2 сохранены (`CREATE OR REPLACE`, идемпотентно). ACL: EXECUTE только
  `authenticated` (anon revoked). Win Wizard передаёт owner (`AssigneeSelect`) + шаблон.
...
- `public.notify_deal_won()` + триггер **`trg_notify_deal_won`** _045_ (applied) —
  SECURITY DEFINER, **plain `AFTER UPDATE ON projects` + `WHEN` (НЕ `AFTER UPDATE OF
  status`)**: `status` дерайвится BEFORE-триггером `trg_sync_deal_stage_fields` из
  `stage_id`, поэтому `UPDATE OF status` не сработал бы. При переходе в `won` вставляет
  `notifications` type=`deal_won`, recipient=`owner_id`, actor=`auth.uid()`,
  entity=`projects`/`NEW.id`. EXCEPTION-safe (не блокирует запись). REVOKE anon/PUBLIC.
...
### Лента (112 `20260808204308` + 113 `20260808222500` + 114 applied `20260809083732` + 115 **applied 2026-08-09 `20260809092051`**) — сборка на сервере, страницами, с фильтром по видам, от сущности до организации
...
- **`public.entity_timeline(p_entity_type text, p_entity_id uuid default null,
  ...
  ref_id uuid, parent_type text, parent_id uuid, payload jsonb)`. `language sql`,
  **STABLE**, `security invoker`, `search_path=public,pg_temp`, `revoke public/anon` →
  ...
  а с 115 — ещё и `useRecentActivity`, который питал два org-виджета.
- **`p_entity_type` — четыре значения (115): `org | project | company | contact`.**
  При `'org'` предикат сущности в каждой ветке снимается целиком, а `p_entity_id`
  игнорируется (потому и `default null`). ⚠️ **Границу организации держит RLS
  источников, и второй проверки в теле функции НЕТ намеренно** — ни `current_org_id()`,
  ни ручного `org_id = …`: это была бы копия предиката политики, которая разошлась бы
  с ней при первом же изменении RLS. Ровно та причина, по которой функция
  `SECURITY INVOKER` (см. ниже). Второй функции `org_timeline()` с копией тела не
  заводится по той же логике.
  Особые случаи org-режима: ветка `src_projects` **включена** (создание сделки —
  событие org-уровня), транзитивная ветвь `src_activity` **выключена**
  (`p_entity_type in ('company','contact')` — прямая уже отдаёт весь журнал org),
  «своя» ветвь `src_ai` **выключена** (`in ('project','company')`, значения `'org'` в
  `ai_runs.entity_type` не бывает), а «детская» ветвь `src_ai` при `'org'` берёт все
  прогоны (`scope_children` для org пуст).
- ⚠️ **Сигнатура заменялась ЧЕТЫРЕЖДЫ, и каждый раз через `drop`, а не перегрузкой.**
  113 сняла трёхаргументную версию 112, 114 — пятиаргументную версию 113: перегрузка
  дала бы PostgREST `PGRST203` (ambiguous), вызов с пятью аргументами подошёл бы и
  пяти-, и шестиаргументному кандидату. **У 115 причина ДРУГАЯ** — список типов
  аргументов не меняется (`p_entity_id` лишь получает `default`), зато меняется набор
  возвращаемых колонок, а `create or replace` этого не умеет (`42P13 cannot change
  return type of existing function`). Гранты после `drop`+`create` не наследуются и
  повторены явно.
- **`parent_type` / `parent_id` (115) — к чему относится событие.** Приоритет один на
  все ветки: `project` → `company` → `contact`, первый непустой; у `src_projects`
  сделка сама себе родитель; у `src_ai` родитель есть только для прогонов по сделке и
  компании (у прогона по звонку/встрече родителя, который клиент умеет назвать, нет).
  ⚠️ **Порядок в `case` и в `coalesce` обязан совпадать** — разъедутся, и клиент пойдёт
  искать компанию по id проекта. **Имя функция не отдаёт**: резолв `id → имя` делает
  клиент из кэшей `useProjects`/`useCompanies`/`useContacts`, как `useActorMap`
  резолвит актора. `null` — законное значение: у 304 записей журнала из 801 привязки
  нет вовсе. Клиент согласует пару целиком (половина пары = мусор: тип без id дал бы
  ссылку на `/deals/null`, id без типа — подпись без ссылки).
- **`p_kinds` (114, расширен в 115) — фильтр по видам, `null` = все.** Словарь из
  девяти значений: `call | meeting | task | project | activity | ai_run` + **три
  производных** среза внутри `activity_log` по `event_type` — `note`
  (`comment_added`), `stage` (`stage_change`, `stage_changed`), `deleted`
  (`entity_deleted`). Это не отдельные источники: `kind` у таких событий остаётся
  `activity`. Вид, которого нет в списке, схлопывает предикат своей ветки в `false`,
  и планировщик отбрасывает её как one-time filter, не читая таблицу. Значение вне
  словаря не ошибка — оно просто не включает ни одной ветки.
  ⚠️ **Списки `event_type` сведены в CTE `kind_types` (115)**: три вида × две ветви
  union — шесть мест на три списка, и в 114 список заметок уже был написан дважды.
  ⚠️ **Каст `::text[]` у подзапроса обязателен**: `x = any((select arr from cte))`
  парсится как форма `ANY(subquery)` и даёт `42883 operator does not exist:
  text = text[]`.
  ⚠️ **Дубль между SQL и TS остаётся** (`NOTE_EVENT_TYPES` в
  `src/lib/utils/activity-events.ts`): SQL не умеет импортировать TS-константу. При
  добавлении типа заметки править **оба места** — разойдутся молча. Табы дашборда
  перечень типов НЕ дублируют: они шлют имя вида (`['stage']`), а не список событий.
- ⚠️ **114: точная отсечка курсора стоит ВНУТРИ каждой ветки union, а не только
  снаружи, и это обязательная половина `p_kinds`.** В 113 ветка отбирала `limit N` по
  грубой отсечке `ts <= p_before`, а точная (по паре) стояла снаружи; пока веток шесть,
  выброшенное снаружи покрывалось запасом (до 300 кандидатов против 50 в выдаче).
  `p_kinds` этот запас убирает: при `p_kinds = ['task']` работает одна ветка, и если
  курсор попал в середину блока ничьих, снаружи отбрасывается **всё** отобранное.
  Страница выходит короче лимита, клиентский `getNextPageParam` видит
  `length < PAGE_SIZE` и решает, что достигнуто дно — **при живом хвосте, без ошибки и
  без признака**. Сверено на живых данных до применения: у «Хороший вкус — внедрение»
  40 задач с одним `ts`; с отсечкой внутри ветки — **40 задач за 4 страницы по 10, 0
  дублей**, без неё вторая страница приходит **пустой**. Внешняя отсечка оставлена
  страховкой на случай ветки, заведённой без внутренней.
  ⚠️ У `src_meetings` точная отсечка написана **по UTC-выражению**
  (`m.date::timestamp at time zone 'UTC'`), хотя `order by` идёт по сырой `m.date`:
  обе половины пары обязаны жить на той же оси, что и внешняя отсечка; приведение
  строго монотонно, поэтому порядок и пригодность индекса не меняются.
...
- **Лимит — НА СТРАНИЦУ (113), а не на источник**; `p_limit` зажат в `[1, 200]`.
  В 112 он был на источник (как клиентский `PER_SOURCE_LIMIT = 50`), и лента показывала
  «до 50 от каждого источника»: на проде 2026-08-08 в этот потолок упирались 3 сделки
  из 19, у «Стратек — внедрение» 143 задачи против показанных 50, и остальное не
  показывалось и не запрашивалось. Смена состава первой страницы **намеренна** и
  отменяет критерий приёмки S-TL-1 (построчное совпадение с прежней клиентской лентой).
- **Курсор — ПАРА `(p_before, p_before_id)`**, а не один `ts`: события одной секунды
  реальны (159 пар в окне ±5 сек), и по одному `ts` страница либо теряла бы строки
  (`<`), либо повторяла их (`<=`). Отсечка — сравнение **кортежей**
  `(ts, id) < (p_before, coalesce(p_before_id, 'zzzz'))`, что раскрывается в
  `ts < a or (ts = a and id < b)` — ровно обратное к `order by ts desc, id desc`.
  Разложить его на `ts <= a and id < b` нельзя: выбросит все строки старше курсора
  с лексикографически бо́льшим `id`. Сентинел `'zzzz'` страхует ручной вызов с
  `p_before` без `p_before_id`: все шесть префиксов `id` меньше него, отсечка
  вырождается в `ts <= p_before` (страница повторит события этой микросекунды, но
  не потеряет ни одной). Клиент всегда передаёт обе половины.
- ⚠️ **Внутренний `order by` каждой ветки union обязан нести ТОТ ЖЕ тай-брейк, что
  внешний** (`…, ('<kind>:' || id::text) desc`). Ветка отдаёт свои топ-N, внешний
  запрос выбирает из них глобальные топ-N — это корректно только при совпадении
  сортировок. Иначе на ничьих ветка вернёт произвольные N строк, внешний отсортирует
  уже урезанный набор, и часть событий пропадёт **без ошибки и без признака**.
  Ничьи не теоретические: у «Хороший вкус — внедрение» 40 событий с одним и тем же
  `ts`, и до 113 их порядок держался на физическом порядке строк.
...
- ⚠️ **Сортировка — сквозной `order by ts desc, id desc` (113).** Тройка 112
  (`date_trunc('milliseconds', ts)` → ранг источника `ord` → позиция внутри источника
  `seq`) **удалена целиком**, и это не упрощение ради красоты:
  - `ord`/`seq` воспроизводили порядок конкатенации массивов в старом JS-хуке. В курсор
    такой ключ не выражается — он не является функцией возвращаемых колонок.
  - `date_trunc('milliseconds', …)` существовал ради совпадения с `new Date(x).getTime()`,
    который **режет timestamptz до миллисекунд** (на живых данных это разводило три пары
    `task`/`activity` из ста). Сортировки на клиенте больше нет, а усечение прямо вредит
    курсору: две строки с Δ 300 мкс схлопнулись бы в один ключ, и страница «раньше» либо
    потеряла бы одну, либо повторила.

  Порядок ничьих теперь задан **контрактом** (`id` уникален по построению), а не
  физическим порядком строк.
...
- ⚠️ **У задачи две даты** (`created_at` и `deadline`), `ts` события —
  `coalesce(deadline, created_at)`. В 112 отбор топ-N шёл по `created_at` (так делал
  прежний клиентский `fetchTasks`); **113 перевела отбор на ту же ось, что и `ts`** —
  иначе курсор и выборка жили бы на разных колонках, и страница «раньше» теряла бы
  задачи. Под новую ось заведены три индекса по выражению
  `idx_tasks_{project,company,contact}_ts`.
- ⚠️ **`meetings` сортируются по СЫРОЙ `m.date`, а фильтруются по выражению**
  `(m.date::timestamp at time zone 'UTC')`. Приведение строго монотонно, порядок тот же,
  зато `idx_meetings_*_date` остаётся рабочим — по выражению планировщик индекс для
  сортировки не подобрал бы.
- **`actor_id` у `ai_runs` заполнен с 113** (`ai_runs.created_by`, NOT NULL). В 112 там
  стоял `null::uuid` — ровно ради построчного совпадения с прежней лентой.
...
- **13 индексов** миграции — по одному на ветку union, все аддитивные, `concurrently` не нужен
  ...
  `idx_projects_{company,contact}_created`. **113 добавляет ещё 3** —
  `idx_tasks_{project,company,contact}_ts` по выражению `coalesce(deadline, created_at) desc`
  (индекс по колонке для выражения бесполезен); прежние `idx_tasks_*_created` остаются,
  по ним ходит доска задач. Ветка `ai_runs.entity_id in (…)` индексом **не закрыта**:
  `idx_ai_runs_entity` начинается с `entity_type`, а составного `(entity_id, …)` нет.
  Оставлено сознательно — в таблице 21 строка.
- **Долг S-TL-2 закрыт в S-TL-3 (114):** выбранный чип уходит в `p_kinds`, и «Звонки»
  означают звонки со всей ленты, а не из загруженных страниц. Пустой результат при
  выбранном чипе теперь честен. **Остаток:** проп `kindFilter` (набор чипов, который
  объявляет карточка) режет ленту по-прежнему на клиенте — сегодня это no-op,
  единственный родитель перечисляет все шесть видов; карточка с УЖЕ полного набором
  вернула бы себе дефект. Лечится передачей набора в `kinds` хука.
- **S-TL-4 (115) закрыл org-долг.** `useRecentActivity` **удалён**: он читал
  `activity_log` напрямую и питал два виджета — «Последние действия» на дашборде (20)
  и `ActivityWidget` в дровере (5). Оба показывали 801 событие организации из 1510
  (2026-08-09), без звонков, встреч, задач, сделок и AI. Теперь оба читают
  `useOrgTimeline` (обёртка над `entity_timeline('org', …)`), табы дашборда шлют
  `p_kinds`, иконка вида — из общего `lib/timeline/kind-meta.ts` (вторая карта по
  `event_type` жила на дашборде и после переезда стала копией).
  ⚠️ **Realtime покрыт ЧАСТИЧНО, и это записано намеренно.** Прежний хук держал
  `useRealtimeSync('activity_log')`, и запись журнала обновляла виджет сама;
  `useOrgTimeline` эту подписку сохраняет, но новую задачу, звонок или встречу
  realtime не принесёт — они приезжают по `staleTime` или по инвалидации из своих
  мутаций (префикс `['timeline']`). Подписки на шесть таблиц ради виджета в 20 строк
  не окупаются; молча потерять то, что работало, хуже, чем честно покрыть половину.
  ⚠️ **Цена контекста — два списочных запроса на дашборде**: `useProjects` там уже
  поднят, `useCompanies`/`useContacts` — нет (прежний виджет обходился вложенным
  `project:projects(id,name)`). Списки кешируются на всё приложение.
- Шесть новых источников (`stage_transitions`, `messages`,
  `quotes`, `project_files`, `deal_stakeholders`, `project_checklists`) в S-TL-2
...
- **082** _(S-SEC-GRANTS-ROOT — **applied 2026-07-27 `20260727201726`**; статус исправлен гейтом S-TL-1: запись «ждёт гейта» пережила применение)_ — корень темы грантов,
...
- **085** _(S-R2-AI-HARDEN, R2-P1 — **applied 2026-07-28 `20260728085826`**; статус исправлен гейтом S-TL-1)_ — `ai_runs` учится
...
- **087** _(S-R2-FIELD-AUDIT, R2-P2, эпик G1 — **applied 2026-07-29 `20260729063848`**; статус исправлен гейтом S-TL-1)_ — аудит
...
- **090** _(S-R2-WEBHOOK-ACTION — **ПРИМЕНЕНА `20260731055734`** на гейте 2026-07-31;
  неприменённых миграций нет)_ — вебхук как действие движка автоматизаций. Порядок деплоя:
...
- **091** _(S-R2-WEBHOOK-JOURNAL — **ПРИМЕНЕНА `20260731065104`** на гейте 2026-07-31;
  неприменённых миграций нет)_ — журнал доставок: повтор и ретеншн. **Аддитивна**: ни одна функция
...
- **Anti-injection:** транскрипт и данные сделки — только в user-turn внутри
  ...
  только как текст. **Экранирования `<`/`>` во входе НЕТ и не будет** — инъекционный
  смоук 28.07 (9 прогонов, 0 пробитий) показал, что вход чистый, а разметку модель
  генерирует сама; гипотеза «экранировать `dataBlock`» этим смоуком отменена.
- **Проверка формы ответа + один ретрай** _(fix-S-R2-AI-SHAPE, `ai-run/shape.ts`)_ —
  чинит не безопасность, а ДОСТУПНОСТЬ. Модель срывается со структурированного вывода и
  упаковывает правильные данные неправильно: `tasks` строкой вместо массива, `<parameter
  name=…>` и `</client_situation>` внутри строковых значений. Замерено: 1 отказ из 4 на
  инъекционном входе, 2 из 7 живых прогонов на чистом — то есть базовый уровень брака,
  инъекция лишь усилитель.
  — `checkResultShape(input_schema, input)` делает **три** проверки и раскладывает
  претензии по **двум** классам. **Жёсткая** — тип верхнего уровня не совпал с
  объявленным в `input_schema.properties` (клиент такой ответ не разберёт); `type` может
  быть массивом (`['string','null']`) или отсутствовать — такие поля пропускаются.
  **Мягкие** двух видов: пропущенное поле из `input_schema.required` и маркер разметки
  (`<parameter name=`, `</`) внутри строкового значения.
  ⚠️ Проверка `required` добавлена не сразу: исходно фикс считал, что «`required` держит
  сама модель». **Факт это опроверг** — прогон `3d5e6e70` (13.07) вернул один ключ из
  шести, и ретрай случился бы СЛУЧАЙНО, только из-за маркера `</`; без маркера огрызок
  вместо записки прошёл бы с вердиктом «форма годна». Класс мягкий намеренно: претензия
  обязана вызвать ретрай, но не отказ — частичный ответ полезнее пустого экрана.
  Вложенные `required` (внутри `items` массивов) НЕ проверяются: поштучно битые элементы
  уже отсеивает клиентский Zod, дублировать незачем.
  На живых данных (7 строк `ai_runs`, 28.07): 5 чисто, `3a75cfbb` — 1 пропуск (`needs`)
  + 1 маркер, `3d5e6e70` — 5 пропусков + 1 маркер, жёстких претензий 0 нигде.
  — Любая претензия → **ровно один** ретрай с добавкой к user-ходу про форму (не про
  содержание: назови модели претензии — начнёт «чинить» данные вместо упаковки).
  Выбирается попытка без жёстких претензий, при прочих равных — вторая.
  — Жёсткие пережили ретрай → `status='error'`, **`result` не пишется** (до правки мусор
  доезжал до БД). Мягкие → пишем + `meta.shape_warning[]` (срез 10). Второй вызов был →
  `meta.retried = true` — единственный способ померить частоту брака в проде.
  — `input_tokens`/`output_tokens` — **сумма обеих попыток**, иначе метрика стоимости
  прогона занижает расход.
  — Худший случай по времени: 2 × самый долгий живой прогон (26.9 с) ≈ 54 с против
  `STALE_RUN_MINUTES` = 10 мин — запас 11×.
  — `shape.ts` — чистый модуль без Deno/supabase-js, чтобы логику можно было гонять
  тестом (`tests/unit/ai-shape.test.ts`); `supabase functions deploy` собирает его в тот
  же бандл по относительному импорту.
