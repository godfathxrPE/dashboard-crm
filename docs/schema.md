...
> **Applied (в живой БД, ref `uoiavcabxgdjugzryrmj`):** миграции 001–030. Фаза 1 multi-user
...
>
> Собрано интроспекцией живой БД + `supabase/migrations`; актуально на 2026-07-07.
...
- `public.set_org_id()` — `BEFORE INSERT` триггер `trg_set_org_id` на всех
  16 tenant-таблицах (+transcripts/ai_runs с 030); проставляет `NEW.org_id :=
  current_org_id()`, если он не задан явно. **На invitations/notifications НЕ
  вешается** — org_id явный.
