...
> путь в тесте;
> **105 (S-R3-TRUST-1) — ФАЙЛ НАПИСАН, НЕ ПРИМЕНЕНА** (`105_segment_incomplete_deals.sql`,
> ветка `feat/data-trust`): схемы не меняет — только сид-строка в `segments`, шестой общий
> сегмент сделок «Полнота <60%» (см. раздел `segments` ниже). Реген типов не нужен;
...
**Сид 105 (S-R3-TRUST-1) — файл написан, НЕ применена:** шестой общий сегмент сделок —
