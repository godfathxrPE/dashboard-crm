...
> **Applied (в живой БД, ref `uoiavcabxgdjugzryrmj`):** миграции 001–027. Фаза 1 multi-user
...
>
> **Pending:** 028 (ai_summary, ждёт кредитов Anthropic), **029** (автоматизация).
>
...
### automation_rules / automation_runs _(029, pending)_ — автоматизация v1
...
| duration_s | int | |
| ai_summary | jsonb | _028 (pending)_ AI-резюме, см. ниже |
| ai_summary_at | timestamptz | _028 (pending)_ момент генерации |
| created_by | uuid | → profiles |
| **org_id** | uuid | **NOT NULL** |
| created_at / updated_at | timestamptz | |
...
> **`ai_summary` (028, pending)** — формат jsonb:
...
- **Known issue (S27):** IIoT-трек (`StackedPipeline` в ProjectDetail) двигает
  сделку через legacy-колонку `stage`, а не `stage_id` — гейт (на `stage_id`) там
  **не срабатывает**. Закрытие = перевод IIoT-трека на `stage_id` (кандидат в
  хвост S28); реверс-маппинг `stage → stage_id` в БД сознательно НЕ делаем (нет
  колонки-кода на стадиях, только name/order_index). То же ограничение у
  автоматизации S29 (тоже на `stage_id`).

### С S29 (029, pending) — автоматизация v1
...
- **028** _(S28, pending)_ — `ai_summary`/`ai_summary_at` на calls/meetings (jsonb +
  timestamptz, `ADD COLUMN IF NOT EXISTS`). RLS не трогаем. Применяется на гейте
  Cowork вместе с деплоем Edge Function `ai-summarize` (см. ниже). Smoke:
...
- **029** _(S29, pending)_ — `automation_rules` + `automation_runs` +
  `run_stage_automations()` + `trg_zz_run_automations`. Применяется на гейте Cowork
  (при мёртвом MCP — ручной фолбэк, как 028). Ревью: EXCEPTION-политика (глотание +
  вложенные), идемпотентность `automation_runs`, whitelist lane/priority, отсутствие
  EXECUTE/format в теле функции, `org_id = NEW.org_id` явный. Smoke: переход в
  «Подготовка КП» (с закрытыми гейт-требованиями S27!) → задача с правильным
  assignee/deadline + уведомление (если assignee ≠ актор) + строка `automation_runs`;
  повторный пинг-понг стадии → второй задачи НЕТ; `is_active=false` → не стреляет;
  правило с битым конфигом → переход НЕ ломается. Advisors.
