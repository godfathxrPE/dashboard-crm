...
> **125 (S-CONTACT-COMPANY) — НАПИСАНА, НЕ ПРИМЕНЕНА** (файл
> `125_contact_company_capture.sql`). Контакт из Telegram привязывается к компании.
...
> Следующая свободная — **126** (125 занята написанной, но НЕ применённой миграцией;
> последняя применённая — 124). ⚠️ Номер брать запросом к
...
- `tg_apply_capture(p_actor_id, p_draft_id, p_kind default null)` → jsonb _(DEFINER, только
  `service_role`; 110, **+124 ветка `task` — applied `20260819175811`**, **+125 привязка контакта
  к компании — НЕ ПРИМЕНЕНА**)_ — применение черновика: создаёт
