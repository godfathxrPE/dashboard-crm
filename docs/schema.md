...
> **Applied (в живой БД, ref `uoiavcabxgdjugzryrmj`):** миграции 001–024 — S23-мультитенантность, S24 org-scoped RLS, S25 командная видимость + hardening (024 применена 2026-07-05, смоки прошли: convert_lead чужого лида → 42501, лог из service-контекста получает org_id, видимость leads под ролями).
>
> **Pending (написаны, НЕ применены — гейт S26):** **025** (FK `leads.converted_*` → ON DELETE SET NULL) и **026** (notifications + invitations, write-политики memberships + гард последнего owner, notify-триггеры, `apply_pending_invites` в `handle_new_user`, realtime notifications).
>
> Собрано интроспекцией живой БД + `supabase/migrations`; актуально на 2026-07-05.
...
### invitations _(026, pending)_ — приглашения в org
...
### notifications _(026, pending)_ — уведомления «тебе назначили»
...
> **Known issue (фикс — 025, S26):** FK `converted_deal_id` / `converted_company_id`
...
### С S26 (026, _pending_) — write-политики memberships + уведомления/инвайты
...
- `public.notify_task_assigned()` / `notify_project_assigned()` _(026, pending)_
...
- `public.protect_last_owner()` _(026, pending)_ — гард последнего owner
  (см. RLS S26).
- `public.apply_pending_invites(uuid, text)` _(026, pending)_ — SECURITY DEFINER;
...
- **025, 026** _(S26, pending)_ — написаны, применяются на гейте Cowork.
