...
| **Cold review** | Спринт с миграцией, RLS, ролями, деньгами (КП/бюджеты) или удалением данных ⇒ первым пунктом «Твоих действий»: `./.grok/skills/review-diff/scripts/cold-review.sh prepare` → заполнить `## ACCEPTANCE` поведением → `run`. Выход — `_analysis/cold-review-<branch>.md`. Скилл `review-diff` не читает спринт-файл, `_analysis/review-*` и `journal.md` — **без плана**, иначе унаследует его ошибку | Гейт написал план; ошибку плана он не увидит (S-HEALTH-V2-1: «тест по спецификации не ловит дефект спецификации»). Grok делает это для плана — для кода не делал никто |
