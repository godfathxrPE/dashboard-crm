...
| Migrations | Полный ledger — **`docs/schema.md` в репозитории**, не эта таблица. На 2026-08-05: последний номер — **104** (применена гейтом `20260803194153`), файлов в `supabase/migrations/` — 65 (номера в папке теряются: 047 и 088a применены без файла), 060 зарезервирована и не занята. **Номер следующей миграции берётся запросом к `supabase_migrations.schema_migrations`** — не из этой таблицы и не из `ls` папки (069–073 записаны там без числового префикса, греп по номеру их не находит) |
