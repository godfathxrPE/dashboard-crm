...
- [ ] **Секция ТЕСТЫ есть.** Новая функция в `src/lib/` (utils, validators, helpers) — с
      `*.test.ts` в том же спринте, кейсы поведением (вход → выход, границы, повтор).
      UI-only спринт — явная строка «тестов нет: только разметка/стили». Правило из
      `architecture.md`: «если функция тестируема — её место в lib/, не в queryFn хука»
- [ ] Время и рандом — аргументом функции (`now`), не `Date.now()` внутри (журнал: `leadStaleness`)
...
| Шаг | Как | Почему |
|-----|-----|--------|
| Ревью диффа | Читать реальные изменения, а не описание из отчёта; сначала `git log main..HEAD` — в диффе только этот спринт | Отчёт CC — заявка, не доказательство; ветка от невлитой ветки даёт чужие коммиты (журнал 2026-08-24) |
| **Тесты** | В диффе есть `*.test.ts` к новой логике в `src/lib/`, кейсы описаны поведением; либо строка «тестов нет: …» в спринт-файле. Нет ни того, ни другого — 🟡, для расчётов/статусов/конвертаций — 🔴 | Дефекты находились «тестом соседней сущности» (S-TAILS-1) — значит, своего теста не было |
| **Линзы ревью** | Маршрутизация по диффу — таблица в `sprint-prompt-builder`, режим 2. Базовая всегда; `security-privacy-reviewer` — миграции/RLS/роли/Storage; `crm-design-auditor` + `a11y-auditor` — новый экран/темы; `api-architect` — Server Actions/внешние API; `supabase-patterns` — хуки/realtime. В «Что проверено» строка «Линзы: …» | Гейт сверял «дифф = план» и не смотрел сложность, имена, контекст. Линза без результата = пропуск |
| **Cold review** | Спринт с миграцией, RLS, ролями, деньгами (КП/бюджеты) или удалением данных ⇒ первым пунктом «Твоих действий»: `./.grok/skills/review-diff/scripts/cold-review.sh prepare` → заполнить `## ACCEPTANCE` поведением → `run`. Выход — `_analysis/cold-review-<branch>.md`. Скилл `review-diff` не читает спринт-файл, `_analysis/review-*` и `journal.md` — **без плана**, иначе унаследует его ошибку. Скилл и скрипт лежат в репо (`.grok/skills/review-diff/`), запуск требует grok CLI на машине (`GROK_BIN` или `~/.grok/bin/grok`) — без него шаг выполняет владелец | Гейт написал план; ошибку плана он не увидит (S-HEALTH-V2-1: «тест по спецификации не ловит дефект спецификации»). Grok делает это для плана — для кода не делал никто |
...
### Триаж находок cold review

Вывод Grok — кандидаты, не задачи. Гейт фильтрует их памятью проекта, в этом порядке:

1. **Нет воспроизводимого «Отказа»** (конкретный вход → неверный результат) — отбросить
   не разбирая. «Может привести к проблемам» разбору не подлежит.
2. **Сверка с `learnings.md` и «Что уже починено» в `CLAUDE.md`.** Осознанное решение
   проекта — отбросить; описанная грабля, в которую наступил код, — severity вверх.
3. Оставшееся: 🔴 — блокером в «Твои действия»; 🟡/⚪ — в отчёт строкой
   «принято» / «отклонено, потому что …».

**Одна и та же находка, отклонённая дважды, — не ошибка Grok, а дыра в памяти:**
инвариант нигде не записан. Строку в стоп-лист `review-diff` и в `learnings.md`
тем же PR.
...
- **Исключение — документная правка** (STATUS, `learnings.md`, `journal.md`, `_analysis/`): её ведёт `scripts/docs-pr.sh` целиком, включая пуш и авто-мерж; ручной цикл остаётся для кода и миграций.
- **После каждого сквош-мержа — `gh pr list --state open`.** Сквош оставляет ветку живой
  и внешне «непримёрженной», а её файлы застывают на состоянии до последующих PR: мерж
  такого дубля откатывает всё, что легло в `main` после. Закрывать в тот же день. Так
  вошёл #56 и так чуть не вошёл #58 — детали и правило про `--auto` в
  `references/learnings.md`, «Однострочники процесса».
...
### Состояние проекта — STATUS.md

- **`crm-architect/STATUS.md` — единственный носитель состояния** («в работе / очередь /
  отложено / закрыто»). Живёт только в репо, **в раскатку скилла и пакет аккаунта не входит**
  намеренно: он меняется каждый спринт, а аккаунтная копия обновляется руками — бандлить
  состояние в скилл значит воспроизвести замершую копию 2026-08-05.
- Событийные документы (спринт-отчёты, гейты, хендоффы) собственных списков «что осталось»
  **не ведут** — только ссылку на STATUS и номер его ревизии. Список долга внутри хендоффа —
  дефект ревью: 22.08 три копии состояния разошлись по ревизиям и дали двойной счёт.
- Сверка с git: `scripts/status-check.sh` — каждый PR, смерженный после последнего тега,
  обязан быть упомянут в STATUS. Расхождение — ненулевой выход, чинить сразу.
  **Закрывающий PR вносит СВОЙ номер в STATUS до мержа** — иначе он же и краснеет на
  следующем прогоне. Исключение автоматическое: PR, который правит только сам
  `STATUS.md`, сверка пропускает (иначе каждая правка реестра требует следующей).
- **Свой закрывающий PR вносится в реестр ВТОРЫМ коммитом, после `gh pr create` и до мержа.**
  Номер до создания PR неизвестен: он сдвигается любым PR, созданным между. Пропуск своего
  номера (#52) и угаданный номер — одна ошибка с разных сторон.

### Протокол закрытия спринта

Порядок фиксированный, выполняется на гейте после приёмки:

1. Спринт-отчёт → `_analysis/sprint-S-*.md` + копия в Claude Project (событие, больше не правится).
2. Урок спринта → `references/journal.md`; переиспользуемое правило → строка в `learnings.md`
   со ссылкой на журнал (правило двух файлов — выше).
3. **Обновить `crm-architect/STATUS.md`**: пункт из «В работе» → таблица «Закрыто» (с номером
   PR); новые долги — в «Очередь»/«Отложено» со ссылкой на событие; поднять ревизию и дату
   в шапке.
