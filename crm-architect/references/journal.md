...
**Цвет полотна дизайн-инструмента — не фон приложения.** В `--bg` уехал `#EDE9E1` — фон
Claude Design ВОКРУГ артборда. Сам артборд был `linear-gradient(180deg,#FFFFFF,#EFEBE8)`.
Тема вышла темнее и minimal (L .841), и ауры (L .866) при L .817 — владелец сформулировал
это как «ты фон намеренно затемнил». Переносить из макета надо фон АРТБОРДА, а если он
градиент — брать светлоту, а не крайнюю точку.
