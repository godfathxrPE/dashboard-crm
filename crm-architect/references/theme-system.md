...
**Тем 8** (AUDIT C удалил `scandi`/`paper`/`sand`; **M1 добавил `t-minimal`**; **S-LIME-TOKENS-1 (2026-09-05) добавил `t-lime` и сделал её дефолтом; S-THEME-COBALT-1 (2026-09-24) заменил её на `t-cobalt`**).
...
| `t-cobalt` | «Cobalt» | **`#275DE5` (кобальт)** | **Дефолт с 2026-09-24** (PR #119; до него — `t-lime`, PR #45, решения `claude/decisions-lime-theme-2026-09-05.md`). Light, непрозрачная. Каркас лайма сохранён: фон `#FAF8F5` почти белый (карточку отделяют тень и подложки зон), `--surface2` УТОПЛЕН (темнее `--bg`), материал шага — сплошной `#17171C`, радиусы 12/20, Inter `cv11`, актив нава — сплошная пилюля без маркера, рельс без разделителей. Сменился акцент: кобальт и заливка, и текст — белый на нём 5.55:1 (`--on-accent #FFFFFF`), к `--bg` 5.24:1, `--accent-text #1D4BC0` 7.03. Костыли лайма (`on-accent` на `bg-accent`, `.text-accent !important`, тёмная `entity-tile`) сняты. **«Инфо»-синий уведён в циан** `--blue #007E9C` / `--blue-text #006882`, иначе акцент и звонки — один цвет. `--zone-work` — 8% акцента (кобальт тёмный, 14% давали тяжёлую зону); ΔE «Работа ↔ Контекст» 5.7. Метка материала шага `#93B7FF`. Решение 24.09: лайм не подходит CRM; превью трёх вариантов — артефакт «Замена лайма» |
...
**Дефолт — `t-cobalt`** (`DEFAULT_THEME`) с 2026-09-24; до него `t-lime` (с 2026-09-05), ещё раньше `t-aura`. Сохранённый `t-lime` — неизвестный ключ: FOUC-гард и `resolvePersistedTheme` уводят его в дефолт штатным путём (тест `theme-store.test.ts`). Порядок `THEMES` = порядок `cycleTheme`, `t-cobalt` стоит первой.
...
непрозрачны). `t-washi`/`t-fuji`/`t-minimal`/`t-cobalt` — светлые непрозрачные (`--glass-blur: none`).
...
--zone-work-tint:      var(--accent);   /* четыре темы уводят его в --purple, см. ниже */
...
непрозрачен, но при почти белом `--bg` (как в `t-cobalt`) даёт базу СВЕТЛЕЕ фона — зона
...
переопределяют `--h-zone` / `--h-ring` / `--h-chip-ink`. Первые потребители — зоны карточки
сделки (S-DEAL-ZONES-1A, #53).

#### ⛔ Тон «Работы» — `--zone-work-tint`, а не `--accent` напрямую (#55)

Формула `--zone-work: color-mix(--accent 14%, base)` молча предполагала, что акцент темы —
**свободный тон**. Это было верно в лайме и верно в кобальте (ΔE «Работа ↔ Контекст» 5.7 при 8%). ΔE (CIE76, Lab) между подложкой «Работы» и
ближайшей соседней плашкой; порог различимости крупных плашек ≈ 5:

| тема | пара | было | стало (`--purple`) |
|---|---|---|---|
| `t-tidal` | Работа ↔ Риски «в норме» | 1.8 | 7.2 |
| `t-washi` | Работа ↔ Риски «киснет» | 2.8 | 5.0 |
| `t-aura` | Работа ↔ Контекст | 3.3 | 8.7 |
| `t-minimal` | Работа ↔ Риски «в норме» | 4.9 | 8.8 |
| `t-fuji` | Работа ↔ Контекст | 5.0 | 4.1 — хуже, не трогаем |

> **Подложка или текст, выведенные из `--accent`, обязаны быть проверены на совпадение
> с семантическими цветами темы.** Акцент свободен не везде: в `t-washi` он равен `--red`,
> в `t-tidal` и `t-minimal` попадает в зелёный, в `t-aura` неотличим от нейтрали. Тон зоны
> «Работа» задаётся токеном `--zone-work-tint` в кросс-темном блоке рядом с зонными
> токенами, а **НЕ** в блоке темы: кросс-темный блок стоит выше всех определений `.t-*`
> в `globals.css`, специфичность у них одинаковая (0,1,0) — и `--zone-work-tint`,
> поставленный внутрь `.t-*`, молча перебьёт политику порядком следования.

`--purple` выбран потому, что это единственный палитровый тон без семантического алиаса:
`danger`/`warning`/`success`/`info` заняли red/yellow/green/blue. `t-fuji` в подмену не
входит — там `--accent` и `--blue` одно значение, и purple делает пару хуже; тема остаётся
слабой, это её известный долг (уголки `::before/::after`, 2.99:1).

Eyebrow зоны идёт тем же выбором — токен `--zone-work-ink`, и в четвёрке тем он уходит
на **`--text-dim`, а не `--purple-text`**: purple-текст на этой подложке даёт 4.13–4.73,
а подпись 11px требует 4.5. Замер после правки — **4.64–7.52 по всем восьми темам**.
Иначе в washi «РАБОТА» была бы набрана красным и читалась как сообщение об ошибке.

### ⚠️ Примитив `.entity-tile`

Плитка сущности в шапке карточки записи (компания, сделка, контакт). База в
`@layer components`: `--accent-l` + `--accent-text`. Тема может перевернуть приём (так делал `t-lime` до 24.09: тёмная плитка с лаймовой
иконкой; в `t-cobalt` снято — кобальт держит базовый приём, 7.45:1). **Цвет иконки не задаётся на самой иконке**: она наследует `color` плитки, поэтому
...
- **рельс** — `bg-surface` на `<aside data-app-nav>`; washi/fuji/minimal/cobalt перебивают;
- **пилюля** — layered-дефолт `.nav-active { background: var(--accent-l) }`.
  ...
  unlayered-правила `t-cobalt` и `t-minimal`. У остальных объявленный непрозрачный
