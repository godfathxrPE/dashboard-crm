...
Три случая 05–06.09 одной формы: утилита `.text-accent` резолвилась в `--accent` мимо
проверенных токенов (иконка 1.10:1 при зелёном аудите); скрипт мерил подложку нава, когда
носителем состояния был левый бордер (три ложных FAIL); `formatContactNameShort` работала по
спецификации, тесты зелёные, на экране «- Н.». Ни в одном случае реализация не была
сломана — ложной была ПОСЫЛКА, на которой строился замер. Реализацию проверяет машина,
посылку не проверяет никто, кроме автора. Журнал: «S-LIME-TOKENS-1», «S-FORMAT-1».
