...
**Третий случай: ОЧЕРЕДЬ ЭПИКА, выведенная из документа решений, ошибается в ОБЕ стороны.**
Разбор 06.09 снял S-DEAL-DATA-1 целиком — и оба вывода оказались неверны по-своему: три
его пункта были давно закрыты чужими спринтами (лишняя работа, если бы спринт поставили),
а четвёртый — роли по воронке — сняли зря (потерянная работа: возражение спорило со
словарём должностей, тогда как спецификация W10 требует ожидаемый состав контура, другую
сущность с похожим именем). Проверка «сделано ли уже» ловит только первую половину: она
не спрашивает, ЧТО именно пункт требует. **Очередь эпика строится по спецификации,
сверенной с кодом** — не по документу решений и не по ревью макета, — **и живёт в STATUS**;
документ карты приводит её один раз, чтобы показать зависимости, и при расхождении STATUS
новее (карта `_analysis/deal-v2-gap-2026-09-06.md`, спецификация `_analysis/deal-v2-spec.html`).
