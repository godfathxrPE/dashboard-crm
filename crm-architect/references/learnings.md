...
- **`add column` проверяется ГРАНТАМИ, а не только политиками.** При колоночных грантах (`role_column_grants` вместо `role_table_grants`) новая колонка в список не попадает и отдаёт 42501 при живых и корректных RLS. Один запрос на гейте объясняет класс отказов, который иначе читается как «RLS сломался» (журнал 2026-09-08).
...
- **Аргумент «констрейнт уронит legacy-строки» проверяется ЗАПРОСОМ, не рассуждением.** Гейт S-DEAL-ORG-1 отклонил CHECK «rejected ⇒ причина» ссылкой на существующие NULL — отклонённых КП в проде оказалось ноль. Правило 0 («трогаешь показ поля — разведуй данные») действует и на гейт. Окно для констрейнта на новой колонке закрывается само, первой же живой записью (журнал 2026-09-08).
...
- **Отменено 2026-08-29:** git из Cowork звать МОЖНО — мост умеет unlink после явного запроса права на удаление в папке. Без этого первый же коммит оставляет `HEAD.lock` и `tmp_obj_*`, и следующая операция падает; зависший лок от прошлой сессии снимается после проверки `pgrep git`. Пуш и `gh` из Cowork по-прежнему недоступны: сети нет.
- **`gh pr merge --auto` в этом репо не работал до 06.09:** `allow_auto_merge` был выключен, команда возвращала `Auto merge is not allowed for this repository`, а PR молча оставался открытым — за вечер накопилось три висящих. Вместе с правилом «ветка свежее base» это давало цепную немержабельность: любой мерж в `main` ронял остальные PR. Оба флага (`allow_auto_merge`, `allow_update_branch`) включены 06.09; отставшую ветку обновлять `gh pr update-branch <N>`, не force-push.
- **Взведённый `--auto` отменяется только `--disable-auto` или закрытием PR — «попросить закрыть» недостаточно.** 06.09 устаревший PR с ранее включённым автомержем влился сам после мержа соседнего и откатил уже принятый виджет (#56). Авто-мерж живёт на стороне GitHub и срабатывает молча, как только сойдутся проверки и base. Снимать явно: `gh pr merge --disable-auto <N>` или `gh pr close <N>`; перед тем как счесть PR безопасным — `gh pr view <N> --json autoMergeRequest`. Журнал: «эпик зон карточки сделки».
- **PR на ветке, содержимое которой уже влито сквошем, обязан быть закрыт в тот же день.** Сквош оставляет ветку живой и внешне «непримёрженной», а её файлы застывают на состоянии до последующих PR — мерж такого дубля откатывает всё, что легло в `main` после. Так вошёл #56 (откатил кольцо) и так чуть не вошёл #58 (откатил бы zone-work-tint). Проверка после КАЖДОГО сквош-мержа: `gh pr list --state open` — закрыть всё, что стоит на только что влитой ветке. Журнал: «эпик зон карточки сделки».
- **Уточнено 2026-09-05:** право на удаление запрашивается ОТДЕЛЬНО и в этой сессии запрошено не было — `git status` из Cowork оставил `.git/index.lock`, и у владельца упал `git commit` («Another git process seems to be running»). Пока право не запрошено, любую git-команду на чтение звать как `git --no-optional-locks …` (`status`, `log`, `diff`, `ls-files` — все они иначе трогают индекс). Залипший лок снимает владелец: `pgrep -l git` → пусто → `rm -f .git/index.lock`.
- `skill-deploy.sh` из Cowork раскатывает в `$HOME` СЕССИОННОЙ VM, а не в `~/.claude/skills` на машине — «раскатано» из Cowork не значит раскатано, и `skill-verify` там же зелён ложно. Полезен только побочный эффект: пакет `<skill>.skill` ложится в корень репо и годится для загрузки в аккаунт.
...
- Контракт в репо не смеет ссылаться на путь из `.gitignore`: `crm-architect/SKILL.md` предписывал звать `.grok/skills/review-diff/scripts/cold-review.sh`, которого в репо не было — шаг невыполним для всех, кроме владельца, и молчат и CI, и `status-check`. Игнорировать состояние (`.grok/*`), а не всю папку с активом. Журнал: «chore/review-diff-in-repo», 2026-08-29.
...
### ⛔ Supabase MCP `generate_typescript_types` ≠ `supabase gen types` CLI — файл им НЕ подменять
Соблазн закрыть реген с гейта (у моста нет сети для `npx`) выглядит рабочим: тул отдаёт те же
~4200 строк с новой таблицей. **Отличие ровно одно, но фатальное: MCP отдаёт ТОЛЬКО схему
`public`** — нет ни блока `graphql_public`, ни его ключа в `Constants`. Санити-проверка
`scripts/gen-types.sh` требует маркер `graphql_public` и отвергла бы такой файл; написана она
ровно для этого класса подмен. Вырезать из вывода один блок таблицы тоже нельзя:
`supabase.gen.ts` правится ТОЛЬКО регенерацией.
⚠️ Прочие расхождения (`PostgrestVersion` 14.4 → 14.5, дженерики в скобках
`TableName extends (… : never) = never`) признаком чужого генератора НЕ являются: реген
настоящим CLI 07.09 дал ровно их же — это новая версия `supabase@latest`, приезжающая первым
же регеном после долгого перерыва. Отличать по составу схем, а не по версии.
Журнал: «гейт apply 130», 2026-09-07.
...
- **Поле, выбираемое по статусу, перечисляет ВСЕ значения enum; ветка «иначе» — решение, а не хвост.** `stampedAt = accepted ? accepted_at : sent_at` печатал дату отправки под подписью «Отклонено»: исключения нет, дата просто неверная. Комментарий над строкой описывал две ветки из трёх — третья проехала и мимо кода, и мимо ревью (журнал 2026-09-08).
...
### ⛔ Календарная дата ВЫВОДИТСЯ через `formatCalendarDate`, а не `formatDateNumeric`
Календарная дата — это `'YYYY-MM-DD'` без момента времени: колонка `date`, значение
`<input type="date">`, датированный снапшот справочника. `new Date('2026-08-03')` по спеке
ECMAScript — **UTC-полночь**, а `format` из date-fns печатает в ЛОКАЛЬНОЙ зоне: при
отрицательном смещении дата уезжает **на сутки назад**. Замерено: `America/New_York` и
`America/Los_Angeles` → «02.08.2026», MSK и UTC → «03.08.2026». `formatCalendarDate`
(`lib/utils/dates.ts`) добавляет `T00:00:00` БЕЗ `Z` — локальная полночь, стабильно везде.

Тот же корень, что у бага деления сутками выше, но другое место: там ломалась АРИФМЕТИКА,
здесь ломается ВЫВОД. Для `timestamptz` хелпер не нужен — момент времени есть, сдвига нет
по определению (`created_at` в «Сводке» намеренно оставлен на `formatDateNumeric`).

Дефект уже жил в проде: `DealSummaryCard` печатал так дедлайн сделки, и `try/catch` вокруг
вызова не защищал — исключения нет, дата просто неверная. Нашёлся не глазами, а по ходу
соседней правки. **Тест на зону обязан быть проверен падением наивной реализации** —
`process.env.TZ` действует не во всяком рантайме, и зелёный тест бывает зелёным впустую.
Журнал: «S-DEAL-CHZ-1».
...
### ⚠️ Подпись над СРЕЗОМ окна — свой агрегат по срезу, не агрегат родительского окна
`DealPulse` считает один `longestSilence` на все 30 дней; тепловая полоса рисует только
последние 14 из них. Заголовок «Дни без активности · N дн.» над этой полосой обязан брать
разрыв, посчитанный ВНУТРИ 14 дней (`silenceWithin(days, 14)`), а не 30-дневный
`longestSilence` — иначе подпись называет число, которого на самой полосе не видно (разрыв
мог случиться за пределами среза). Общее правило: любой виджет, рисующий ЧАСТЬ окна домена,
заводит свою функцию агрегата по срезу, а не переиспользует агрегат всего окна для частичной
подписи. Журнал: «S-DEAL-PULSE-1».

---

## CSS & Themes

- **Регексп `\.t-<тема>\s*\{` ловит групповой селектор, если имя темы стоит в нём ПОСЛЕДНИМ.** `audit-contrast.py` так мерил `t-minimal` по `:root` вместо её блока — девять фантомных FAIL в рабочей теме владельца. Остальные имена группы стоят перед запятой и не совпадают (журнал 2026-09-08).
...
### ⛔ Замер бывает зелёным потому, что меряли НЕ ТО — проверять посылку, а не результат
**«Правило соблюдено, ступень взята не та.»** Шесть случаев 05–06.09 одной формы: утилита
вместо токена (`.text-accent` резолвится в `--accent`, иконка 1.10:1 при зелёном аудите);
пилюля вместо бордера (скрипт мерил подложку нава, носителем состояния был левый бордер —
три ложных FAIL); `formatContactNameShort` по спецификации, тесты зелёные, на экране «- Н.»;
заливочная семантика на подложке, выведенной из той же семантики (`--warning` на зоне
«Риски» — 1.54 в `t-fuji`); акцент как тон зоны (2.8 в washi, 4 темы из 8).
**Шестой случай — проверка смотрела не на тот АРТЕФАКТ:** `skill-deploy.sh` собирал пакет
относительно `$PWD`, и раскатка из worktree клала `<skill>.skill` туда же; локальная копия
при этом обновлялась верно (её путь считается от `$HOME`), `skill-verify` был зелён — он
сверяет источник с локальной копией и на пакет **не смотрит вовсе**. В основном чекауте,
куда владелец ходит за файлом для загрузки, пакет остался от 05.09 при ревизии 34
в источнике. Зелёная проверка тут не ошибалась — она просто не покрывала третий экземпляр
памяти. Починено с двух сторон: путь пакета привязан к корню основного чекаута
(`--git-common-dir`, НЕ `--show-toplevel` — в worktree тот отдаёт корень worktree),
и `skill-verify` теперь распаковывает пакет и сверяет его с ИСТОЧНИКОМ, а не только
локальную копию: ловит расхождение содержимого, отсутствие пакета и — предупреждением —
пакет старше источника по mtime. **Мораль шире случая: проверка обязана покрывать
КАЖДЫЙ экземпляр артефакта, а не самый удобный.**
**Плюс обратный случай:** `*-text` вместо палитры там, где 1.4.11 вообще не применялся —
сегменты кольца не необходимы для опознания состояния, его несёт каждая строка списка;
замер верный, требование не то, ценой стал вид. Ни в одном случае реализация не сломана —
ложной была ПОСЫЛКА замера. Реализацию проверяет машина, посылку не проверяет никто.
Журнал: «S-LIME-TOKENS-1», «S-FORMAT-1», «эпик зон карточки сделки».

### ⛔ Форматтер полей проверяется на ФАКТИЧЕСКИХ данных, до написания спецификации
«Пустое» в импортированной базе почти никогда не `NULL`: у пяти контактов `last_name = '-'`
(К7 пишет дефис в пустое поле), `trim()` считает это значением, и правило «сокращаем, когда
есть обе части» собирало «- Н.». Тем же запросом видно, чем чинить нельзя: в базе есть
настоящие «Цой» и «Гук», отсечка по длине была бы регрессом; сравнение с заполнителями —
ТОЧНОЕ, строкой целиком (дефис внутри «Римский-Корсаков» законен).
**В РАЗВЕДКУ спринта, который трогает показ поля, обязателен запрос «что реально лежит в
колонке»** — распределение коротких и пустых значений, а не только тип колонки.
Журнал: «S-FORMAT-1».

### ⚠️ `:empty` на обёртке спасает гостя, а не владельца
`<div class="mt-2 empty:mt-0">{slot}</div>`: у не-owner слот отдаёт `null`, div пуст,
отступ снят. У owner внутри лежит скрытая `h-0 opacity-0` кнопка — div НЕ пуст, `mt-2`
остаётся. Правку «скрыть, но оставить фокусируемым» проверять на роли, ради которой она
делалась. Журнал: «S-FORMAT-1».

### ⛔ Новая тема принимается ГЛАЗАМИ — `audit-contrast.py` этого не заменяет
`0 FAIL` по восьми темам и следом четыре дефекта, найденных владельцем. Все одного класса:
**токен объявлен верно, но до элемента не доходит.** Скрипт сравнивает токены между собой и
не знает про Tailwind-утилиты, layered-правила и инлайн-стили. Приёмка новой темы = скрипт
**и** живой дев: фон, актив нава, разделители, плитки, кнопки. Журнал: «S-LIME-TOKENS-1».

### ⛔ Акцент, непригодный как цвет текста, ломает утилиты — а не только кнопки
Tailwind генерирует `.text-accent` / `.bg-accent` из `colors.accent = var(--accent)`. Если
`--accent` светлый (лайм `#C9F25A` на белом = **1.29:1**), объявленного `--accent-text` мало:
утилита резолвится в `--accent` и красит 321 место, `bg-accent text-white` — ещё 82. Нужны
ДВА правила темы: `.t-X .bg-accent.text-white → var(--on-accent)` (прецедент — тёмные темы,
строки ~1550) и `.t-X .text-accent → var(--accent-text)`. Симптом, который ловится только
глазами: иконка в плитке `bg-accent-l` даёт 1.10:1. Журнал: «S-LIME-TOKENS-1».

### ⛔ `.nav-active` живёт в `@layer components` — токены `--sidebar-*` его не гасят
Дефолт даёт `border-left: 3px solid var(--accent)` + `background: var(--accent-l)` +
`.lucide { color: var(--accent) }`. Компоненты `--sidebar-indicator`/`--sidebar-active-text`
не читают — их читает только CSS темы, unlayered-правилом
`.t-X aside[data-app-nav] .nav-active` (образец — `t-minimal`). Гасить боковой маркер через
`border-left: none`, а не `transparent`: иначе активный пункт остаётся сдвинутым на 3px.
Три линии рельса заданы ИНЛАЙН в `TextNavSidebar` (`borderBottom` логотипа, `borderTop`
кнопки, `borderWidth` на `aside`) — снимаются только `!important`. Журнал: «S-LIME-TOKENS-1».

### ⛔ `color-mix` от `--surface` в glass-темах отдаёт ПРОЗРАЧНОСТЬ
У frost/aurora/tidal `--surface` = `rgba(255,255,255,0.07)` и родственные;
`color-mix(in srgb, var(--surface) 55%, var(--bg))` даёт alpha ≈ 0.49, и всё производное
наследует её. Для непрозрачной подложки брать **`--popover`** (S-UI-POLISH-1: в светлых
темах = `--surface`, в тёмных — solid hex). Журнал: «S-LIME-TOKENS-1».

### ⛔ Перенося решение из соседней темы — переносить и УСЛОВИЕ, при котором оно верно
Фон `t-lime` правился дважды и дважды мимо (`#EDE9E1` → `#F2EFE9` → `#FAF8F5`), потому что
обе итерации опирались на правило minimal «фон темнее белого, чтобы карточки отделялись без
тяжёлых теней». В minimal карточки лежат прямо на фоне; в lime их отделяют тень
`--shadow-card` и цветные подложки зон, а фон почти не виден. Значение проверяется скриптом
и кажется проверенным — посылка, из которой оно выведено, не проверяется ничем.
Побочно: из дизайн-инструмента переносить фон АРТБОРДА, а не полотна вокруг него; если
артборд градиент (`#FFFFFF→#EFEBE8`) — плоский токен берётся с той стороны, которая видна
на первом экране. Журнал: «S-LIME-TOKENS-1».

### ⚠️ При почти белом `--bg` порядок поверхностей переворачивается
`--surface2` (hover, чипы) обязан быть УТОПЛЕННЫМ — темнее `--bg`, а не светлее, как в
остальных светлых темах. По той же причине `--zone-base` считается от `--bg` с подмесом
`--text`, а не от `--surface`/`--popover`: непрозрачная поверхность при белом фоне даёт базу
светлее фона, и зона перестаёт читаться как подложка. Журнал: «S-LIME-TOKENS-1».
...
- **Виджет, обязанный быть видимым при свёрнутой секции, получает ТРЕТИЙ слот примитива.** Не `children` (не смонтированы до первого раскрытия, прячутся `hidden`) и не `summary` (рендерится внутри `<button>` — вложенная кнопка ломает клавиатуру). `alwaysVisible` в `CollapsibleSection` — журнал 2026-09-08.
- **Игнор `.next/**` в `eslint.config.mjs` якорится от корня и не ловит вложенные сборки** (`.worktrees/*/.next/**` от брошенного worktree). Локальный `eslint .` краснеет, CI зелёный — он делает свежий чекаут. Два случая за два дня, оба лечили сносом каталога; `'**/.next/**'` закрывает класс (журнал 2026-09-08).
...
### ❌ Скрипты репо исполняются на bash 3.2 — проверять надо там, а не в Linux-контейнере
`/bin/bash` в macOS — 3.2 (лицензия GPLv3). Под `set -u` развёртывание ПУСТОГО массива
`"${ARR[@]}"` там «unbound variable», в bash 5 — легально. `skill-deploy.sh` с пустым
`CORE` упал на трёх скиллах из пяти, хотя из Cowork (Linux, bash 5.1) раскатывался зелёным.
Писать `${ARR[@]+"${ARR[@]}"}`. Общее правило: проверка скрипта в Cowork-мосте НЕ равна
проверке на машине владельца — среда другая (см. также раскатку в `$HOME` сессионной VM).
Журнал: «fix/skill-deploy-bash32», 2026-08-29.

### ❌ `now` параметром у обёртки не спасает, если вложенная функция читает часы сама
`getDealSignals` принимала `now` и честно передавала его в арифметику просрочки, но
`nextStepSignal` звала `getDealHealth(project)` — а та внутри делала `new Date()`.
Тест `deal-signals.test.ts` был зелёным в день написания и покраснел через двое суток:
пять падений, ветка гейта краснела с 25.08 и блокировала мёрж, причём правки в PR были
только в памяти. Это тот же дефект, что `leadStaleness` в 2b, — значит правило
«время аргументом» относится ко ВСЕЙ цепочке вызова, а не к публичной функции.
Проверка дешёвая: тот же вход с `now` и с `now + 7 дней` обязан давать разные вердикты.
Отдельно: тест, привязанный к дате «сегодня» в момент написания, — мина с задержкой;
красный CI при диффе из одних `.md` читать как «протухший тест», а не «сломал я».
Журнал: «fix/deal-health-now», 2026-08-29.
...
### ⛔ `--auto` при `BEHIND` не сработает никогда и об этом молчит
Взведённый авто-мерж — не обещание мержа. Если ветка отстала от `main`
(`mergeStateStatus = BEHIND`), а branch protection требует актуальности, PR висит с
зелёным CI бесконечно: #74 простоял час. Любой опрос состояния PR читает `state` И
`mergeStateStatus` одним вызовом (`gh pr view N --json state,mergeStateStatus`); при
BEHIND — `gh pr update-branch N`, и только один раз, иначе цикл кормит сам себя. BEHIND
возникает всякий раз, когда между созданием PR и его мержем в `main` попал другой PR.
Журнал: «S-DOCS-PR-1», 2026-09-07.

### ⛔ Если успешный путь проверяется гонкой — не хватает режима в инструменте
Позитивный прогон `docs-pr.sh` шёл так: настоящий PR со взведённым авто-мержем плюс
сторож, снимавший мерж раньше срабатывания. Успело — но ставкой был `main`. Признак
универсальный: когда проверка требует УСПЕТЬ отменить последствие, чинить надо
инструмент (флаг «сделай всё, кроме необратимого» — `DOCS_PR_NO_AUTOMERGE=1`), а не
поднимать аккуратность проверяющего. Журнал: «S-DOCS-PR-1», 2026-09-07.

### ⚠️ Спецификацию проверять на ПОСЛЕДНЕМ реальном PR того же класса
«В документной правке только `.md`/`.html`» звучало безупречно и отвергло бы PR #72,
смерженный тем же утром: гейтовой правкой там были шапки миграций (`.sql`, «НЕ применена»
→ `applied`). Выдуманный пример подтверждает правило, реальный — ловит. Журнал:
«S-DOCS-PR-1», 2026-09-07.

### ⛔ Многострочные команды владельцу — без `-m "…"`: zsh раскрывает `!` в буфере
`git commit -m "… только !important …"` упал с `zsh: event not found: important`. History
expansion срабатывает при чтении ВСЕГО вставленного буфера, а не построчно, поэтому zsh
отбросил и предшествующий `git add` — владелец получил «всё выполнено» при пустом индексе
и потерянном коммите. Многострочное сообщение отдавать через `git commit -F <файл>`
(файл кладёт гейт) или `git commit -F - <<'MSG'` с КАВЫЧКАМИ вокруг разделителя. Команды,
меняющие индекс, сцеплять: `git add … && git commit …` — иначе commit уходит в пустой
индекс молча. То же для любого `` ` `` в тексте. Журнал: «S-LIME-TOKENS-1».
...
### ⛔ «Прятать, а не размонтировать» без «после первого раскрытия» = обязательная загрузка
Требование `hidden` вместо размонтирования защищает от перезапроса ПРИ РАЗВОРАЧИВАНИИ, но
буквально исполненное заставляет содержимое монтироваться всегда: свёрнутая доска задач
рендерилась на каждой карточке сделки и держала данные, хотя раньше грузилась по клику на
вкладку. Формулировать в два такта: до первого раскрытия содержимого нет в DOM, после —
`hidden` и никакого размонтирования. Журнал: «S-DEAL-LAYOUT-1», 2026-09-07.

### ⛔ Критерий смоука целит в признак ИМЕННО этой правки, не соседний
Для ленивого монтажа предложенная проверка «в Network нет запросов задач, пока секция
свёрнута» ложна: сводку строит тот же хук, запрос есть всегда — смоук показал бы
«не починили» на исправном коде. Наблюдаемый признак — отсутствие содержимого в DOM.
Перед смоуком спросить: что именно меняет правка и где это ВИДНО.
Журнал: «S-DEAL-LAYOUT-1», 2026-09-07.

### ⚠️ Противоречие внутри спринт-дока решается устройством кода, а не счётом упоминаний
Две формулировки про delivery-доску спорили друг с другом; исполнитель выбрал ту, что
встречалась дважды. Проверяемый ответ был рядом: у `delivery`/`internal` зон нет вовсе,
секцию зоны некуда ставить. Вычитывать спринт на противоречия до отдачи, а найденное в
работе — разрешать чтением кода и предъявлять причину. Журнал: «S-DEAL-LAYOUT-1».

### ✅ Визуальную приёмку делает гейт, если Claude in Chrome подключён
`find` по описанию элемента вместо координат (скриншот падал по CDP-таймауту на dev-сервере),
`get_page_text` показывает чего в DOM НЕТ — именно это доказывает ленивый монтаж. Смоук
трёх раскладок и деплинка занимает минуты и не ждёт владельца. Журнал: «S-DEAL-LAYOUT-1».

### ⛔ `RETURNING` печатает вердикт, а не проверяет его
Смок заморозки `org_id` был написан как `update … returning 'org_id ПЕРЕПИСАН — ДЕФЕКТ'`:
литерал вернулся ровно потому, что UPDATE затронул строки, — независимо от того, что триггер
сделал со значением. Вердикт «дефект» на исправном коде. `RETURNING` отдаёт ПРОВЕРЯЕМОЕ поле,
вывод делается по значению. Симметрично ложно зеленеет и обратный случай.
Журнал: «гейт apply 130», 2026-09-07.

### ⛔ Смок «упало ли?» бесполезен против триггера, который не падает
`freeze_org_id()` не бросает исключение, а молча пишет `new.org_id := old.org_id`: UPDATE
проходит успешно, ошибки нет, значение остаётся прежним. До FK на подменённую организацию
дело при этом не доходит вовсе. Любой инвариант, который держит ТИХИЙ триггер, проверяется
чтением значения после записи, а не наличием ошибки. Журнал: «гейт apply 130», 2026-09-07.

### ✅ Ролевые смоки: `admin` и `viewer` эмулируются, иначе не проверяются никогда
В живой БД есть только `owner` и `manager`, второй организации нет — а политики пишутся под
все четыре роли в каждой миграции. Приём: `begin; update memberships set role='viewer' …;
set local role authenticated; set local request.jwt.claims '{"sub":"…"}'; <проверка>; rollback;`.
«Чужая org» — claims с несуществующим `sub`: `current_org_id()` даёт NULL, RLS отдаёт ноль
строк. `trg_protect_last_owner` подмене роли менеджера не мешает.
Журнал: «гейт apply 130», 2026-09-07.

### ⛔ Зеркало CHECK'а проверяется сквозным вызовом, не сравнением определений
`leads_decision_role_check` держат копией `deal_stakeholders_role_chk` не ради симметрии
текста, а потому что расхождение роняет `convert_lead` ошибкой 23514. Сверка двух
`pg_get_constraintdef` проверяет написанное, а не работающее: смок обязан прогнать реальную
...
- Сид с условием (`direction`, `entity_type`) доказывается тем, куда он НЕ сел: пустота у delivery-воронок и есть обратная совместимость. Запрос — `left join` от словаря, чтобы обе половины были видны сразу. Журнал: «гейт apply 130».
...
- **Правку размеров отдавать таблицей «было → стало», а не прилагательными.** «Развести маркеры, увеличив расстояние» прочиталось как рост строки списка вместо роста шкалы и стоило круга CC. «Крупнее», «просторнее», «развести» читаются двояко и прочитаются не так (журнал 2026-09-08).
- **Карта «спека против кода» видит только пронумерованные виджеты и только функции.** Шапка сделки без номера `W` выпала из таблицы и из очереди целиком; W1 помечен «есть» при несовпадающем виде. Сверять карту с макетом ГЛАЗАМИ, а не по наличию компонентов (журнал 2026-09-08).
- **Длинные heredoc в интерактивный zsh не отдавать** — паста рвётся, шелл виснет в `heredoc>`, особенно после `&&`. Файл кладёт мост или `pbpaste > файл` (журнал 2026-09-08).
- **`cold-review.sh` из основного чекаута требует `--branch`**: без него текущая ветка = база = `main`, отказ «нечего ревьюить». Grok при этом остаётся на `main` и читает файлы ветки через `git show` — соседний код по main, что и требуется (журнал 2026-09-08).
- **`gh pr merge --delete-branch` удаляет ветку и локально, и на origin** — хвостовой `git branch -D` в том же блоке падает «branch not found» (журнал 2026-09-08).
...
### ⛔ Макет СТАРШЕ решений по нему — перед копированием элемента проверить, не отменён ли он
В макете «Сделка v2» вердикт стоит и под шагом, и в виджете здоровья — это закрытая F-01,
аудит 04.09 её снял. Сегментация в макете живёт в горизонтальной пятиполосной шкале, а не
в кольце; Р3 убрал её как дубль списка — перенеся сегменты в кольцо, вернули убранное и
испортили вид. Макет — снимок замысла ДО решений по нему; решения его переопределяют, а
не наоборот. Журнал: «эпик зон карточки сделки».

**Зеркальный случай: документ решений тоже стареет.** Очередь в
`claude/decisions-lime-theme-2026-09-05.md` составлялась по внешнему review, который
смотрел на макет, а не на код: к моменту постановки три пункта из четырёх в S-DEAL-DATA-1
были закрыты спринтами месячной давности — `postpone_count` не понадобился (счёт переносов
идёт из аудита полей 087), у `stage_dwell_defaults` консьюмеров оказалось два, а
`events.effects` уже доезжал до ленты как `activity_log.payload.changes`. Документ решений
старше кода ровно так же, как макет старше решений. **Перед постановкой спринта из старого
документа решений — грепом проверить каждый пункт, не сделано ли уже.** Цена ошибки та же,
что у макета: спринт возвращает снятое или пишет второй раз написанное.

**Третий случай: ОЧЕРЕДЬ ЭПИКА из документа решений ошибается в ОБЕ стороны.** Разбор
06.09 снял S-DEAL-DATA-1 целиком, и оба вывода оказались неверны по-своему. Три его
пункта были давно закрыты чужими спринтами — поставленный спринт переписал бы сделанное.
Четвёртый — роли по воронке — сняли ЗРЯ: возражение спорило со словарём должностей, а
спецификация W10 требует ожидаемый состав контура по типу воронки, другую сущность с
похожим именем; пункт вернулся из «Отложено» как S-DEAL-ROLES-1. Проверка «сделано ли
уже» ловит только первую половину ошибки — она не спрашивает, ЧТО именно пункт требует,
и потому пропускает снятое зря. **Очередь эпика строится по спецификации, сверенной с
кодом, и живёт в STATUS**; документ решений и карта дельты приводят её один раз, чтобы
показать зависимости, и при расхождении STATUS новее (карта
`_analysis/deal-v2-gap-2026-09-06.md`, спецификация `_analysis/deal-v2-spec.html`).
...
- **Проекция, которую домен посчитал, а компонент не использует, — мёртвый экспорт с тестом.** `stacks` в `deadline-track.ts` тестируется двумя кейсами и не читается никем: компонент группирует по дню сам и богаче (худшее состояние + `+N`), и эта логика теста не имеет. Либо домен отдаёт то, чем пользуются, либо не отдаёт (журнал 2026-09-08).
