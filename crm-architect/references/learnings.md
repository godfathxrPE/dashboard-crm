...
- **`useMemo(..., [deps])` с `new Date()` внутри замораживает «сейчас».** Счётчик «N дн. без касания» пересчитывался только при изменении ленты: на вкладке, открытой через полночь, печаталось вчерашнее число. Для суточных счётчиков ключ зависимости — календарный день (`YYYY-MM-DD` в нужной зоне), а не время и не пустой массив (журнал 2026-09-09).
...
- **⛔ Замер цвета ПОСЛЕ runtime-смены темы врёт, если не выключить `transition`.** В Chromium смена цвета через `var()` под `transition` не перерисовывает элемент: `getComputedStyle` отдаёт цвет ПРЕДЫДУЩЕЙ темы. Проверено прямо — с `transition: none` кнопка даёт `#454D5E`, без неё `#D2D2D6`. Любая контрастная таблица, снятая переключением класса темы на `documentElement` без этой поправки, недостоверна целиком (журнал 2026-09-09).
- **Материал, инвертирующий подложку, проверяется машинным обходом узлов, а не списком токенов.** Пять забытых токенов подряд (`--border` → `--border-input` → `--sheet-plate-border` → `--sheet-btn-border`), и первые четыре доставались по одному руками. Обход: все узлы внутри виджета, фактические `color`/`background-color`/`border-*`/`outline`/`fill` + `:hover`/`:focus-visible` из CSSOM, сверка с эффективным композитом подложки. Критерий приёмки самого обхода — он обязан найти уже известный дефект; иначе ему нельзя верить (журнал 2026-09-09).
- **Правило темы по `[data-*]` бьёт слоевое правило материала по специфичности, и порядок в файле не спасает.** `.t-aura [data-card]` (0,2,0) против `.glass-sheet` (0,1,0): в aura материал не рендерился вовсе, три отчёта подряд несли для неё таблицу контраста того, чего на экране нет. Лечится парой селекторов `[data-card].glass-sheet` (приём `[data-card].co360-hot` из того же файла). Проверять — замером `getComputedStyle` в КАЖДОЙ теме, не чтением CSS (журнал 2026-09-09).
- **Расчёт композита намечает, замер утверждает.** Модель «тон × α поверх зоны + блик» разошлась с рендером на 0.2–0.3 по тексту и ровно на 1.0 по заливке (3.06 расчётных против 2.01 фактических) — ошибка качественная, а не в процентах. Значения принимать по рендеру (журнал 2026-09-09).
- **«Контракт уже объявлен во всех темах» ≠ «материал уже есть».** `--glass-*` объявлены в восьми темах, но в пяти светлых `--glass-bg: var(--surface)` — стекло штатно вырождается в лист, и виджет на нём читается обычной белой карточкой. Прежде чем строить материал на существующем контракте, посмотреть, во что он вырождается в каждой теме (журнал 2026-09-09).

- **Любое «акцентом» из макета проверять расчётом по восьми темам ДО кода.** Спека рисуется в лайме, где `--accent` — цвет; буква аватара на `--text` проходит ТОЛЬКО там (13.89), в остальных 1.63–3.61. Второй случай после точки «сегодня» в таймлайне. Замена — `--bg`: 12.1–16.9 везде, в тёмных темах пара инвертируется сама (журнал 2026-09-08).
...
### ℹ️ Тем 8, дефолт `t-aura` (AUDIT C + M1; `t-lime` добавлена позже)
...
### ⛔ Насыщенный цвет — на разметку, не на площадь; цвет спеки проверять на живых пропорциях
Токен, верный в макете как полоска, кричит, когда живые данные делают из него плиту: карта
воронки брала `--yellow`/`--red`/`--text` на 100% под пересвет-полоску макета, а на нормах
3–5 дн. пересвет — 2/3 столбика (до 14:1 в тёмных темах). Заливки площадей — альфа-токены
(`--profile-*`: 55% светлые, 42% тёмные), насыщенный цвет — линиям, контурам, числам. Перед
спринтом с графикой — посчитать, какую ДОЛЮ площади цвет займёт на живых данных.
Журнал: «2026-09-24 · S-STAGE-PROFILE-1».

### ⚠️ Сверка «дефект темы вылечен» — проверять лечение, а не только причину
В washi `--accent = --red`; лечение — переопределение `--mark-today` после `:root`. #108 его
снёс, а сверка входа #115 проверила только причину («accent = red — да») и регрессию не
увидела. При сверке темы: `grep` самого переопределения, а не факта, ради которого оно есть.
Журнал: «2026-09-24 · S-STAGE-PROFILE-1».
...
- **Мигание ЧИСЛА и мигание ПОДПИСИ — разной цены.** Пока `useQuotes` грузился, шапка показывала бюджет и подменяла его суммой КП: старая цена как текущая. Аналогия с `DealOrgBlock` (там появляется лейбл) не работает — денежное значение ждёт ответа запроса, а не показывает промежуточное (журнал 2026-09-08).
...
- **Dev-прогон в worktree оставляет стейл `.next`, и следующий `npm run build` там падает на `ENOENT .next/server/pages-manifest.json`.** Это не регрессия правки: снести `.next` и собрать заново (журнал 2026-09-09).
...
09.09 это правило было пропущено при живой записи в learnings: блок владельцу собирался
по памяти. Вывод — **`gh pr update-branch N` ставить в цепочку мержа ВСЕГДА**: на свежей
ветке он безвреден, а на отставшей экономит круг. И: `--auto` асинхронна, поэтому команда
мержа и уборка worktree НЕ МОГУТ идти одним сообщением — 09.09 уборка отработала до мержа
именно так. Журнал: «S-DEAL-QUOTE-REASON-1».
...
тестовой записи и сверять порядок колонки после возврата. ⛔ Пишущий смок (DnD меняет колонку, `completed_at`) — только на тестовой компании, не в
клиентской сделке: на гейте #113 это нарушено. Журнал: «2026-09-24 · S-DEAL-BOARD-1».
...
- **Пропадающий виджет читается как поломка, пустой — как ответ.** Таймлайн без задач возвращал одну строку и выглядел сломанным; ось с отмеченным сегодня сама сообщает «впереди чисто». Пустое состояние рисует всю конструкцию, пусто только поле данных (журнал 2026-09-08).
- **Второй форматтер одной величины заводится с ГРАНИЦЕЙ в доке.** `formatBudgetFull` — полные разряды, место применения ровно одно (шапка сделки), и это записано в комментарии функции: появится второе место — повод спорить, а не копировать вызов (журнал 2026-09-08).
...
### Лента с плановыми датами содержит будущее — «первое сверху» ≠ «последнее»
Прежде чем брать «первое событие» из отсортированного по дате списка, проверить, есть ли
среди источников тот, у кого дата ПЛАНОВАЯ, а не фактическая. В ленте сущности это задача:
её дата события — `deadline ?? created_at`, поэтому задача со сроком на следующей неделе
стоит первой строкой. Виджет «последнее событие» без отсечки `date <= now` показывает то,
чего ещё не было. Журнал: «S-DEAL-EVENT-1».

### Правило «`now` аргументом» — про чтение часов, а не про наличие параметра
Функция, все сравнения которой относительны собственного якоря, текущего времени не знает,
и параметр `now` ей не нужен. Придуманная неиспользуемому параметру работа — верхняя
отсечка «на случай сбоя часов» — тихо съедает настоящие данные при рассинхроне часов
браузера. Если параметр оказался лишним, его убирают, а не наполняют смыслом. Журнал:
«S-DEAL-EVENT-1».

### Кастомный класс из `@layer components` не получает вариантов Tailwind
`.bg-surface-hover`, объявленный руками в CSS, даёт только базовый класс: `hover:`,
`focus:`, `disabled:` к нему не применяются — варианты Tailwind генерирует лишь для имён
из палитры конфига. `hover:bg-surface-hover` не работает нигде и молча: 134 места в 75
файлах. Проверять генерацией (`tailwindcss` на пробнике с одним классом), а не чтением.
Журнал: «S-DEAL-EVENT-1».

То же и с МОДИФИКАТОРОМ ПРОЗРАЧНОСТИ: `bg-surface-hover/40` не порождается тоже, а
ручное правило объявлено без дроби — такие места не «работают без ховера», они прозрачны
всегда (ещё 7 мест, найдены сверкой входа ORG-2). Найдя, что вариант не генерируется,
проверить и `/NN`: механизм один, симптомы разные. Замена ищется по двум образцам —
`hover:bg-…` и `bg-…/`, один `sed` второй не ловит. Журнал: «S-DEAL-ORG-2».

### Замена аргумента по имени не находит вызовы с выражением
Убирая параметр из сигнатуры, искать вызовы не грепом по имени переданной константы: там,
где на её месте стоит выражение, греп промолчит, а компилятор потом упадёт. Считать
аргументы с учётом вложенных скобок или прогонять `tsc`. Журнал: «S-DEAL-EVENT-1».

### Токен `var(--другой-токен)` на `:root` резолвится до навешивания темы
`--mark-today: var(--accent)` в `:root` подставляется на том узле, где объявлен. Класс
темы висит на `documentElement`, поэтому в приложении всё верно — но стенд, который
раскладывает темы вложенными `div`'ами, покажет всем один цвет корня (лайм «терракотовым»).
Это артефакт стенда, а не правки; тот же механизм у `--danger`. Переопределять такой токен
в теме — только ПОСЛЕ `:root` по порядку в файле: специфичность `:root` и `.t-*`
одинаковая (0,1,0), и объявление внутри блока темы выше по файлу проигрывает молча.
Журнал: «S-DEAL-ORG-2».

### Инструмент аудита проверяется на собственную правоту раньше, чем ему верят
`audit-contrast.py` искал блок темы регекспом без `(?m)^` и ловил групповой селектор
вместо блока: девять FAIL из двенадцати были фантомными, и скрипту перестали верить
целиком — вместе с настоящими находками. Диагноз ставится прогоном (`m.start()` → номер
строки), не чтением регекспа. Чиня инструмент, считать «до» и «после» на ОДНОМ дереве:
иначе не отличить «FAIL сняты правкой скрипта» от «FAIL сняты правкой кода». И ревьюеру:
изменение числа проверяемых пар — первое место, где смотрят дифф, подкрутить
классификатор дешевле, чем чинить причину. Журнал: «S-DEAL-ORG-2».

### Констрейнт на поле, которого не видно в UI, защищает данные впустую
132 сделала `quotes.rejection_reason` обязательной при `rejected` — а рендера у поля не
оказалось ни одного (греп по `src/**/*.tsx`: только запись и зануление). Инвариант в БД
без места на экране даёт «срез» лишь тому, кто ходит в базу SQL'ем. Вводя обязательность
поля, тем же спринтом проверять, ГДЕ оно читается: ревью по диффу этого не ловит — старый
рендер в дифф не попадает. Журнал: «S-DEAL-ORG-2» (смок).

### Пакет памяти собирается ПОСЛЕ мержа, и `skill-verify` этого не ловит
`skill-deploy.sh` пакует то, что лежит в рабочем дереве. Запуск в цепочке с `git pull`
не гарантирует ничего, если auto-merge правки ещё не отработал: 09.09 `worktree-isolation.skill`
собрался из старого текста. `skill-verify` при этом ЗЕЛЁН — он сверяет три копии между
собой (репо · `~/.claude/skills/` · `.skill`), а они были согласованно старыми, и про
аккаунт он не знает по построению. Проверять НАЛИЧИЕ правки, а не согласованность:
`unzip -p <skill>.skill '*SKILL.md' | grep -c '<маркер>'`, и тот же `grep -c` ставить
в цепочку раскатки — он остановит её, пока PR не в `main`. Журнал: «S-DEAL-QUOTE-REASON-1».

### Правка `tailwind.config.ts` не доезжает до работающего dev-сервера
Конфиг читается один раз при старте `next dev`. `globals.css` и компоненты обновляются
через HMR, а новое имя в палитре — нет: утилита (`bg-mark-today`) просто не существует,
фон не заливается, инлайновый `color: var(--on-accent)` даёт белый текст на белом.
Симптом выглядит как сломанный виджет из последнего спринта, а не как отсутствующий
класс. Отличать по цвету ДО правки: будь код старым, элемент красился бы прежним токеном;
пустота означает «класса нет». Лечится рестартом dev, при упорстве — сносом `.next`.
Журнал: «S-DEAL-QUOTE-REASON-1».

### ⛔ Утверждение об ОТСУТСТВИИ нельзя делать из усечённого чтения
09.09 гейт трижды за один заход заявил «этого в проекте нет», и все три раза ошибся
на инструменте, а не на факте: список пропсов читался через `head -20`, а искомый `map`
стоял тридцать шестой строкой интерфейса; `grep -c -- "--shadow-accent"` вернул 1, и
единица была прочитана как «токен есть», хотя означала «объявлен ровно в одном месте» —
внутри `.t-lime`; спека-HTML признана «облегчаемой» по числу `<div>`, хотя контент лежал
в `<script type="__bundler/template">`.

Правило: **отрицание требует полного чтения области, а не среза.** Интерфейс — целиком
(`sed -n '/^export interface X/,/^}/p'` без `head`), токен — `grep -n` с контекстом
объявления, а не `-c`, файл — по структуре, а не по счётчику тегов. Цена ошибки
асимметрична: ложное «есть» ловится компилятором, ложное «нет» уезжает в спринт-файл
и в STATUS как факт. Все три поймало внешнее ревью (Grok), а не сам гейт.
Журнал: «S-STAGE-PROFILE-1».

### Git в репо владельца через мост — только `--no-optional-locks`
26.09 обычный `git status` из Cowork-VM оставил в репо владельца `.git/index.lock`: git
пытается обновить индекс, берёт лок, а снять его VM не может — удаление в подключённой папке
запрещено. Любая следующая git-команда владельца упала бы с «Another git process seems to be
running». Правило: чтение git через мост — `git --no-optional-locks …` (status, log, diff,
cat-file); пишущих команд гейт в репо владельца не делает вовсе. Если лок всё же остался —
`mv .git/index.lock _to_delete/` (папка в `.gitignore`), не просьба удалить руками.
Журнал: «fix-S-STAGE-PROFILE-3».

### Состояние «загружается» — свойство хука, не домена
Когда отображение зависит от асинхронных настроек, «ещё не знаем» не кладётся в доменный тип
и не подменяется дефолтом: хук отдаёт `null` / `pending`, чистая функция остаётся чистой.
Дефолт до ответа — не фолбэк, а неправда с задержкой (норма 5 → порог группы 14, «киснет» →
«в норме»). Точка готовности одна на ключ (`useStageNormInputs`), а не проверка у каждого
потребителя — забыть её тогда нельзя. Сводку (вердикт worst-wins) при неполных входах не
показывать вовсе: отдельные сигналы честны по отдельности, сводка — нет.
Журнал: «fix-S-STAGE-PROFILE-2», «fix-S-STAGE-PROFILE-3».

### «Настройка переопределена» проверяется значением, а не ключом
26.09 гейт #127 записал долг P-7 «у организации ключи переопределены» по `jsonb_object_keys`
— ключи есть. Значения оказались равны дефолтам (`reconnect_days = 21`, `completeness_rules = {}`),
и видимого дефекта не было вовсе; спринт на ~2 ч чуть не ушёл в работу. Правило: прежде чем
оценивать вред «дефолта до загрузки», сравнить ЗНАЧЕНИЕ настройки с дефолтом в коде. Совпадает —
...
### Статус из снапшота с датой протухает — «стартует» считать от `now`
06.09 спринт-файл велел подсветить группу ЧЗ со `status: 'starting'`; к запуску 26.09 у всех
шести таких групп дата старта уже прошла, и подсветка усилила бы ложный «горячий лид». Правило:
поле справочника, чей смысл зависит от сегодняшней даты («стартует», «истекает», «новый»),
не хранится статусом — выводится чистой функцией с `now` аргументом; разведка спринта сверяет
такие поля с календарём, а не с кодом. Журнал: «S-DEAL-CHZ-2».

### База `color-mix` в плане — сверить на тёмной теме до спринта
26.09 план CHZ-2 велел смешивать тёплую плашку от `--bg` (твёрдый во всех темах — `--surface`
в glass-темах даёт прозрачность). В тёмных темах `--bg` темнее `--surface` карточки, и подсветка
стала провалом (P-8). Правило: выбирая базу смеси для подсветки, в плане указать, как она ляжет
