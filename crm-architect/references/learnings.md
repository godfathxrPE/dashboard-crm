...
### ℹ️ Цвет полотна дизайн-инструмента ≠ фон приложения
Из Claude Design в `--bg` уехал фон ВОКРУГ артборда (`#EDE9E1`), сам артборд был
`linear-gradient(180deg,#FFFFFF,#EFEBE8)` — тема вышла темнее и minimal, и ауры. Переносить
фон артборда; если он градиент — брать светлоту, а не крайнюю точку. Журнал: «S-LIME-TOKENS-1».
