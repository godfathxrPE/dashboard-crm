...
4200 строк с новой таблицей. Дифф против боевого файла показал другое — вывод другой версии
генератора: пропала схема `graphql_public` целиком, `PostgrestVersion` 14.4 → 14.5, дженерики
переписаны лишними скобками (`TableName extends (… : never) = never`). Санити-проверка
`scripts/gen-types.sh` требует маркер `graphql_public` — она отвергла бы такой файл, и
написана ровно для этого класса подмен. Вырезать из вывода один блок таблицы тоже нельзя:
`supabase.gen.ts` правится ТОЛЬКО регенерацией. Реген остаётся за владельцем на машине с
сетью; гейт доводит миграцию до `applied` и оставляет стаб на месте, честно назвав хвост.
