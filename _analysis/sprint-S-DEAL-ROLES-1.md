...
Для дефолтной org и двух deal-воронок. Матчить воронки ПО ИМЕНИ через подзапрос, как
делает seed 027; воронка не нашлась — пропуск, не ошибка.
...
export interface RoleSlot {
  ...
  hint: string | null;
  isRequired: boolean;
  filled: StakeholderRow | null;  // кто закрыл слот
}
...
Логика:
- ожидания есть и `missingRequired` непуст ⇒ сигнал `bad`, `label` из `hint`
  обязательной роли («ЛПР не в контуре — ключевой риск стадии»), cta ведёт к виджету;
...
**6.3.** Пустой слот: пунктирная рамка 1.5 `--line-4` (см. спеку W10), аватар-плюс,
