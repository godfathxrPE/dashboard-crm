...
## Технические хвосты
- [ ] **Четыре мёртвые realtime-подписки** (находка разведки S-LEAD-HUB-2b, 2026-08-10):
      в publication `supabase_realtime` НЕТ `contacts`, `companies` и `activity_log`,
      хотя `use-contacts.ts`, `use-companies.ts`, `use-activity-log.ts` и
      `use-entity-timeline.ts` на них подписаны — каналы открываются, событий не
      приходит никогда. Чинится одной строкой на таблицу, но включение `activity_log`
      оживит ленты во всём приложении (лента сущности, org-лента, дровер): отдельное
      решение со своим смоком по нагрузке, а не «заодно» с 121.
- [ ] **`getNextActionOverdueDays` (deal-health) занижает просрочку на день в UTC+**
      (найдено тестами S-LEAD-HUB-2b): сравнивает ЛОКАЛЬНУЮ полночь сегодня с
      UTC-полуночью `next_action_date`, в MSK теряет 3 часа и `floor` съедает сутки —
      вчерашний шаг сделки показывается как «просрочен 0 дн.». У лида это уже
      исправлено (`getLeadActionOverdueDays` через `diffDaysKey`); в сделке правка
      трогает DealFocusPanel/TodayView/фильтры и требует своего смока.
