...
**Этот спринт правит НЕ репозиторий, а память** — две её части:

1. `~/.claude/skills/crm-architect/` — скилл, общая память проекта. Её читают
   **все** сессии: и Claude Code, и Cowork на гейте.
2. `~/.claude/projects/-Users-oleg-Downloads-dashboard-crm/memory/` — журнал
   Claude Code (`project-gotchas.md`, `s-ui-clarity-1.md` и прочие). Её читает
   **только Claude Code**; Cowork на гейте её не видит вовсе — мост не выходит за
   пределы папки проекта.

Обе вне git, поэтому ревью пройдёт чтением файлов, а не диффом. Ветку в проекте
создавать не нужно; коммита в конце нет.

⚠️ Из второго пункта следует правило, которое этот спринт обязан закрепить:
**всё, что должно быть видно на гейте, живёт в скилле.** Знание, оставшееся
только в журнале CC, для проверяющего не существует.
...
## ЗАДАЧА 0: разведать журнал Claude Code

Прежде чем править скилл — прочитать вторую память и понять, что в ней есть:

```bash
ls -la ~/.claude/projects/-Users-oleg-Downloads-dashboard-crm/memory/
wc -l ~/.claude/projects/-Users-oleg-Downloads-dashboard-crm/memory/*.md
cat ~/.claude/projects/-Users-oleg-Downloads-dashboard-crm/memory/project-gotchas.md
```

Разложить содержимое на три корзины и **привести списком в отчёте** (по 1–2
строки на пункт, с цитатой):

- **A. Переносится в скилл** — общие правила и грабли, полезные любому спринту и
  проверяющему («тема-правила на голом теге», «zustand-блоб в localStorage»,
  «протухший dev-сервер», конвенции). Их место — `learnings.md` скилла.
- **B. Остаётся в журнале** — хроника конкретных спринтов: что делал, чем
  доказал, какие числа получил. Дублировать в скилл не нужно.
- **C. Противоречит скиллу или репозиторию** — факты, разошедшиеся с реальностью
  (устаревшие номера миграций, «тем 6», старый акцент). Такое чинится в обеих
  памятях, а не оставляется «на всякий случай».

Ничего не удалять на этом шаге — только классифицировать.
...
## ЗАДАЧА 6: развести зоны двух памятей

По результатам Задачи 0:

1. **Корзину A перенести** в `references/learnings.md` скилла (дописать к урокам
   из Задачи 5, не плодя дубли: если правило уже есть — не повторять, а уточнить).
2. **Корзину C починить в обоих местах** одинаково: в журнале CC — поправить или
   вычеркнуть устаревший факт, в скилле — записать верный.
3. В начало `project-gotchas.md` (журнал CC) добавить короткий блок о разделении
   зон, чтобы следующая сессия CC не начала снова копить общие правила у себя:

   ```markdown
   ## Где что живёт
   - Архитектура, конвенции, гейт-протокол, темы, общие грабли — скилл
     `crm-architect` (~/.claude/skills/crm-architect/). Его читают и Claude Code,
     и Cowork на гейте.
   - Здесь — журнал спринтов: что делал, чем доказал, какие числа вышли.
   - Правило: если правило пригодится ЛЮБОМУ следующему спринту или проверяющему
     на гейте — его место в скилле. Cowork этот каталог не видит.
   ```

4. В `SKILL.md` скилла, в раздел про гейт, добавить строку: у Claude Code есть
   собственный журнал `~/.claude/projects/<проект>/memory/`, недоступный Cowork;
   на гейте спрашивать отчёт, а не рассчитывать прочитать его самому.
...
# две памяти не противоречат друг другу по ключевым фактам
for f in "001–075" "Тем 6" "C05A2E" "D6006E" "SQL Editor"; do
  printf "%-12s скилл: %s | журнал CC: %s\n" "$f" \
    "$(grep -rc "$f" ~/.claude/skills/crm-architect/ | paste -sd+ | bc)" \
    "$(grep -rc "$f" ~/.claude/projects/-Users-oleg-Downloads-dashboard-crm/memory/ | paste -sd+ | bc)"
done   # ожидание: нули по обеим колонкам
```
...
Отдельно в отчёте: **классификация журнала CC по корзинам A/B/C** (Задача 0) —
это единственный способ показать её проверяющему, который каталог не видит.
