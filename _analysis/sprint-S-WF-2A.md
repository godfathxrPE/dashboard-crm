# Claude Code Prompt — S-WF-2A: Workflow Engine (backend, миграция 050) — v2 ПОЛНЫЙ SQL

> Дизайн — `sprint-S-WF-2-architecture.md`. **Обобщаем S29-движок**. **Миграцию пишешь, НЕ применяешь** (гейт Cowork). Прод ref `uoiavcabxgdjugzryrmj`.
>
> **Нумерация:** файлы в репо до `049_task_dep_created_by_default.sql`; 049 **уже applied** (Cowork, эта сессия — `column_default=auth.uid()` подтверждён). Эта миграция = **`050_workflow_engine.sql`**. На гейте Cowork сверит `schema_migrations`.
>
> **Разведка живой БД выполнена (Cowork), имена зафиксированы ниже — не угадывай:**
> - unique на `automation_runs`: **`automation_runs_rule_id_project_id_stage_id_key`**
> - `notifications_type_check` = `['task_assigned','project_assigned','deal_won']` → +`'automation'`
> - NOT NULL: notifications(`org_id,recipient_id,type,entity_type,entity_id`), activities(`type,title,org_id`)
> - `run_stage_automations` — SECURITY DEFINER, search_path `public,pg_temp`, ACL service_role only (сохранить!)
>
> **Красная линия:** 3 row-триггера (`task_overdue` → S-WF-2C, нужен планировщик). UI — S-WF-2B. `set_field` — только whitelist.

## Файл `supabase/migrations/050_workflow_engine.sql` — целиком
...
> Триггер `trg_zz_run_automations` НЕ пересоздаём. Seed-правила S29 (`stage_entered`/`create_task`) остаются валидны под новым CHECK.

## ГЕЙТЫ CC
```bash
npx tsc --noEmit && npm run build     # 050 — DDL, кода не трогает; ожидаемо 0/0 (build при живом dev — изолированный distDir, как в S-CRIT-PATH)
git diff --cached --stat              # только 050_workflow_engine.sql
```

## КОММИТ
```bash
git add supabase/migrations/050_workflow_engine.sql
git commit -m "feat(workflow): S-WF-2A — обобщение automation-движка (triggers + actions + conditions)"
```

## ЖЁСТКО НЕ ТРОГАТЬ
- НЕ apply 050 (гейт Cowork).
- НЕ трогать `src/` (types/UI/NotificationBell — это S-WF-2B).
- НЕ добавлять `task_overdue` (S-WF-2C, нужен планировщик).
- НЕ расширять whitelist set_field на stage_id/status/type/org_id.
- НЕ переименовывать функцию / пересоздавать триггер.

## ПОСЛЕ ТЕБЯ — Cowork (apply + смок ролями, транзакции с rollback)
1. `list_migrations` → apply 050 → `get_advisors` (новых WARN на automation_* нет).
2. **backward-compat:** seed-правило `stage_entered→create_task` всё ещё создаёт задачу.
3. **status_changed:** правило на `to='won'` → при переходе в won срабатывает notify/activity.
4. **field_changed + conditions:** правило `field='budget'` + condition `budget gte N` → смена budget выше N срабатывает, ниже — нет.
5. **идемпотентность:** повтор UPDATE с тем же trigger_key → run НЕ дублируется.
6. **set_field + re-entrancy:** правило `set_field(next_step)` → поле проставилось, движок НЕ зациклился (wf.ran), UPDATE прошёл.
7. **notify/activity INSERT:** строки реально создались (recipient_id/type/entity_* заполнены; activities.title не NULL).
8. **swallow:** правило с битым cast (`set_field probability='abc'`) не роняет UPDATE и не гасит другие правила.
9. Обновить `docs/schema.md` + skill `references/schema.md` (conditions, trigger_key, CHECK-расширение, движок, +отметить 049 applied).

## VERIFICATION
```
Type Safety:            N/A (SQL)
RLS Coverage:           PASS (политики automation_*/notifications не меняются; runs — DEFINER)
DEFINER/search_path:    PASS (зафиксированы в §5 + REVOKE/GRANT service_role)
Backward Compatibility: WARNING (обобщение — смок п.2; runs.trigger_key backfilled)
Idempotency:            NOT_VERIFIED (смок п.5)
Re-entrancy:            NOT_VERIFIED (смок п.6)
NOT NULL contracts:     PASS (notify/activity INSERT по живым NOT NULL-колонкам)
```
