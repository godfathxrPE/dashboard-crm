...
### Волна 1 — «Быстрые победы» (1 спринт, всё S, поверх готовой схемы)
...
| S-GANTT-VIEW-1 | Gantt read-only, группировка по фазе, milestone-ромбы, зум неделя/месяц (К5). **Scope жёсткий v1** (см. §6) | M | dates |
...
| S-SEGMENTS-1 | Server-side `segments` + RLS (К7) | M |

### Волна 4 — «Качество закрытия + глубина» (по мере надобности)

| Epic | Что | Effort |
|---|---|---|
| S-AI-WRITEBACK-1 | Post-meeting HITL: transcript → предложение полей сделки (К3.5) | M |
| S-WBS-1 | `parent_task_id` + `wbs_code`, tree UI | M |
| S-CHECKLIST-1 | `project_checklists` sign-off (приёмка из xlsx ДО) | M |
| S-ERP-PARITY-1 | ERP spawn/шаблон до уровня IIoT (PM-growth §4) | M |
| S-INTERNAL-1 | Internal templates + участие в Portfolio (PM-growth §7) | M |
...
Это закрывает «Ступень 1» из PM-GROWTH-ZONES (PM видит план и судьбу сделки) почти бесплатно и даёт понять, стоит ли вкладываться в дорогие В2-В3 (Gantt, workflow) — или дешёвых побед уже достаточно.
