 ...
 *
 * Статические потолки (55 + 25 с) сломались на первом же живом прогоне (19.08 11:41),
 * причём по причине, обратной ожидаемой: **чем лучше ищет проход 1, тем вернее
 * промахивается статический потолок прохода 2**. Сорок один источник вместо шести дал
 * черновик втрое толще пробного, упаковщик получил больше входа — и не уложился в срок,
 * рассчитанный по тонкому черновику. Поднять 25 000 до 40 000 значило бы переставить ту
 * же грабку на шаг дальше, поэтому проход 2 получает ОСТАТОК бюджета: быстрый поиск
 * отдаёт упаковщику много, медленный — мало.
 *
 * ⚠️ ЧЕМ ОГРАНИЧЕН БЮДЖЕТ. Предыдущая редакция называла ограничением шлюз Supabase
 * (~90 с на синхронный вызов) — и это было НЕВЕРНО ПЕРЕНЕСЕНО: `ai-run` отвечает
 * `{ run_id }` сразу, а `processRun` живёт в `EdgeRuntime.waitUntil`, то есть шлюзом
 * синхронного ответа не связан вовсе. Реальный потолок — **wall clock воркера: 150 с
 * на Free-тарифе** (Supabase Functions Limits / Background Tasks). CPU-лимит (2 с) нам
 * не мешает: он не считает асинхронный I/O, а у нас всё ожидание — сеть.
 *
 * Отсюда 100 с, а не 150: wall clock считается НА ВОРКЕР, а не на прогон. Воркер мог
 * обслуживать запросы до нас, и сколько от его жизни осталось — узнать неоткуда;
 * пятьдесят секунд оставлены чужой работе и запуску.
 *
 * ⚠️ Бюджет живёт внутри ОДНОГО вызова `callLlmSearch`. Ретрай формы в `ai-run` — это
 * второй вызов со своим бюджетом, и два полных прогона по 100 с в 150 с воркера не
 * влезают. На практике ретрай пропускает проход 1 (черновик переиспользуется) и стоит
 * ~25 с, поэтому риск оставлен осознанно; на платном тарифе (400 с) он снимается сам.
 */
const BRIEF_BUDGET_MS = 100_000;
/**
 * Потолок прохода 1. Замер: 27 с на сорока одном источнике — 50 000 даёт почти
 * двукратный запас и оставляет упаковщику ровно столько же в худшем случае
 * (50 + 50 = 100 при потолке воркера 150).
 */
const SEARCH_PASS_TIMEOUT_MS = 50_000;
/**
 * ПОЛ срока упаковщика, а не порог отказа.
 *
 * Прежняя редакция при остатке меньше минимума отказывалась звать проход 2 вовсе — то
 * есть мы придумали стену и сами в неё упирались: на бюджете 82 с это означало отказ
 * при поиске длиннее 64 с. Симметрии между исходами нет: **превысить оценочный бюджет
 * не смертельно, отказать — смертельно точно**, потому что отказ гарантированно теряет
 * уже оплаченный поиск. Бюджет у нас оценочный (см. выше про воркер), и упереться в
 * настоящий потолок — случай редкий, а выбросить готовый черновик — случай каждый.
 */
...
export async function callLlmSearch(
  ...
  if (provider === 'openrouter') {
    ...
    // Остаток бюджета, но НЕ МЕНЬШЕ пола: медленный поиск урезает упаковщика, а не
    // отменяет его. Отказ до вызова здесь был бы придуманной стеной — бюджет
    ...
    if (BRIEF_BUDGET_MS - searchMs < STRUCT_FLOOR_MS) {
      // Не ошибка, но и не норма: поиск съел весь бюджет, прогон пойдёт за него.
      console.warn('search pass overran the budget, struct floor applied:', JSON.stringify({
        search_ms: searchMs,
        budget_ms: BRIEF_BUDGET_MS,
        struct_floor_ms: STRUCT_FLOOR_MS,
      }));
    }
