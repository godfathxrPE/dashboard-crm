// supabase/functions/ai-run/chz-groups.ts — S-COMPANY-AI-1 (F2)
//
...
//
// ⚠️ ЗЕРКАЛО: копия `src/lib/data/chz-groups.ts`. Оригинал живёт там, здесь — копия.
...
/**
 * Источники снапшота. Доезжают до экрана подсказкой (`title`) на подписи
 * «Справочник на …» в карточке сделки: «на какое число» видно всегда, «откуда
 * цифра» — по наведению. Списком в рельсе не выводятся намеренно — два URL на
 * каждой открытой сделке это шум, а нужны они в одном сценарии: когда снапшот
 * идут пересверять с честныйзнак.рф.
 */
