-- 125_contact_company_capture.sql — S-CONTACT-COMPANY
--
-- Контакт, созданный из Telegram, привязывается к компании.
--
-- Боевой случай: «Андрей, коммерческий директор, 89113435345, агрохолод» давал
-- контакт БЕЗ компании, а слово «агрохолод» уезжало в `notes`. Две причины, и обе
-- вне модели: в схеме интента `contact` не было поля под компанию (правится в
-- `ai-capture`), а `tg_apply_capture` не писала в `contact_company` вовсе
-- (правится здесь).
--
-- ⚠️ ЭТО ПРАВКА ОДНОЙ ВЕТКИ, А НЕ НОВЫЙ МЕХАНИЗМ. Тело взято из
--    `pg_get_functiondef` живой функции 2026-08-21 и сверено с файлом 124:
--    нормализованные тела совпали побайтно (185 строк, md5 82325aa8…). Ветки
--    `company` и `task` не тронуты; в ветке `contact` дописан блок привязки
--    ПОСЛЕ вставки контакта.
--
-- ⚠️ Аргументы и тип возврата прежние ⇒ `drop function` не нужен, хватает
--    `create or replace` (S-LEAD-HUB-2a).
--
-- Возврат функции не меняется: привязка — побочный факт создания контакта, а не
-- второй исход, который боту надо показывать иначе.
