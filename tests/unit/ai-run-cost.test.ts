...
describe('оценка прогона с веб-поиском', () => {
  ...
  it('считается по замерам, а не по charCount (80К вход + 3К выход + 5 поисков)', () => {
    // Перезамер S-COMPANY-AI-1c по 8 прогонам: вход 75–86К, не 40К.
    // (80 000×$3 + 3 000×$15)/1M = $0.285, плюс 5 поисков по $0.01 = $0.335 → 33.5 ₽
    expect(estimateWebRunCostRub('sonnet').min).toBe(34);
  });
});
