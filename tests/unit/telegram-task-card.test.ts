...
describe('buildTaskCard — не разрешённое показывается с причиной', () => {
  ...
  // ⚠️ S-TG-VOICE-TERMS: «не нашёл» и «не смог посмотреть» — РАЗНЫЕ строки.
  //    Первая — утверждение о справочнике, и человек по ней идёт править своё
  //    сообщение. При сбое запроса он ни при чём, и звать его исправлять свою речь
  //    значит уводить диагностику от настоящей причины.
  it('справочник не прочитался — текст не обвиняет человека в его формулировке', () => {
    const card = buildTaskCard({
      draftId: DRAFT,
      text: 'Позвонить в Мукомол',
      deadline: { iso: null, reason: 'empty' },
      project: { reason: 'error', hint: 'Мукомол' },
    });
    expect(card.text).toContain('Сделка: «Мукомол» — не удалось проверить справочник, привяжите в CRM');
    expect(card.text).not.toContain('не нашёл');
  });
