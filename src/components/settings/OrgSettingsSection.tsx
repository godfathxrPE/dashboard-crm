...
/**
 * Настройки организации (R2-P0-D). Пока одно поле — порог тишины.
 *
 * ⚠️ UPDATE на `organizations` — owner-only (`org_update_owner`). Admin и ниже видят
 * ЗНАЧЕНИЕ, но не форму: пустой disabled-инпут без объяснения читается как баг, поэтому
 * не-owner получает read-only строку с явной подписью «правит владелец организации».
 */
export function OrgSettingsSection() {
  ...
  const {
    ...
    values: { reconnect_days: current },
  });

  // Значение приезжает асинхронно — синхронизируем «чистое» состояние формы,
  // иначе isDirty остаётся true сразу после загрузки.
  useEffect(() => {
    reset({ reconnect_days: current });
  }, [current, reset]);

  const onSubmit = handleSubmit(async (values) => {
    try {
      await update.mutateAsync({ reconnect_days: values.reconnect_days });
        ...
        <form onSubmit={onSubmit} className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              className="w-24 rounded border border-input bg-surface px-2 py-1.5 text-sm text-text-main
