...
// акцент тратится ТОЛЬКО на «сегодня» — бюджет акцента Р2. Берётся он через
...
const STATE_DOT: Record<MarkState, string> = {
  ...
  // «Сегодня» — единственное место акцентного бюджета таймлайна. Двойное кольцо (контур
          ...
          <div className="relative h-[2rem]">
            {track.days.map((day) => (
              <span
                ...
                // WeekLanes. В кобальте это белый (5.55:1), но в темах, где
                // акцент светлый, `text-white` не держит контраст.
                //
