  ...
  return (
    <div className="rounded-lg border border-border bg-surface-hover/40 p-3">
      ...
      <p className="mt-1 text-meta text-text-mute">
        {/* S-COST-TRUTH-1: «разброс попыток» — прежняя правда. Диапазон описывал
            разницу «одна попытка / с ретраем», а после S-COMPANY-AI-1c ретрая почти
            не бывает; теперь границы — крайние замеры ВХОДА (сколько страниц втянет
            веб-поиск), и подпись обязана говорить то же, что считает функция. */}
        Ищет в открытых источниках; в компанию ничего не записывается само.
        Оценка учитывает веб-поиск и разброс объёма найденного; фактическая цена
        прогона показывается рядом со статусом «Готово».
      </p>
