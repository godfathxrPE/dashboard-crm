...
function evalClause(row: SegmentRow, clause: SegmentClause, allowed: Set<string>, todayKey: string): boolean {
...
export function matchSegment(
  ...
  return clauses.every((c) => evalClause(asRow(row), c, allowed, todayKey));
}
...
export function applySegment<T extends object>(
  ...
  const allowed = segmentFieldSet(entity);
  const todayKey = mskDateKey(now);
  return list.filter((row) => clauses.every((c) => evalClause(asRow(row), c, allowed, todayKey)));
}
