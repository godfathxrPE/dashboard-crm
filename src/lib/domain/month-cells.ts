 ...
 * Само правило — в `utils/plural.ts` (S-TASKS-FIX-2 вынес его туда, когда
 * понадобился второй счётчик; две копии правила разошлись бы).
 */
export function pluralEvents(n: number): string {
  return `${n} ${pluralRu(n, 'событие', 'события', 'событий')}`;
}
