 ...
 *
 * Словарь берётся из `STAKEHOLDER_ROLES` и НЕ переписывается здесь списком — поэтому
 * седьмое значение `influencer` (130) приехало сюда само. Продублировать список тут
 * значило бы завести четвёртую точку синхронизации к трём CHECK'ам в БД.
 */
