...
export const companyFormSchema = z.object({
  ...
  //
  // Домен намеренно НЕ сужается до enum названий справочника: справочник —
  // датированный снапшот, и переименование группы сделало бы существующие
  // карточки нередактируемыми целиком (та же логика, что у `inn` выше).
  // Сироты видны на карточке отдельным блоком, а не роняют форму.
