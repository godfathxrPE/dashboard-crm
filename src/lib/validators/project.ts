...
/**
 * Полные разряды — «2 800 000 ₽». Место применения РОВНО ОДНО: шапка сделки
 * (S-DEAL-HEADER-1, спека `deal-v2-spec.html:520–562`).
 *
 * ⚠️ Это НЕ замена `formatBudget` и не второе правило для списков. Компактный
 * формат существует ради плотных списков и карточек, где сумма — одна из десяти
 * строк; шапка же то место, где человек читает цену сделки перед звонком клиенту,
 * и «2,8 млн» теряет там восемь тысяч рублей разницы, которая в переговорах
 * существует. Появится второе место — это повод спорить, а не копировать вызов.
 *
 * Разделитель разрядов — тот же неразрывный пробел, что и в `formatBudget`:
 * `parseBudgetInput` гасит `[\s₽]`, поэтому показанное читается обратно.
 */
export function formatBudgetFull(kopecks: number | null): string {
  if (kopecks == null) return '—';
  const rub = Math.round(kopecks / 100);
  const digits = String(Math.abs(rub)).replace(/\B(?=(\d{3})+(?!\d))/g, NBSP);
  return `${rub < 0 ? '-' : ''}${digits}${NBSP}₽`;
}
