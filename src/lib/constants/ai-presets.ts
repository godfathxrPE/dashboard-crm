...
/**
 * Эмпирика прогонов брифа. **Перезамер 2026-08-04 по 8 прогонам** (гейт, версии
 * промпта 1–2): одна попытка с поиском — 75–86К входных токенов (веб-страницы,
 * втянутые в контекст) и 2–3К выходных при 4–5 поисках. Прежние 40К были выведены
 * по двум прогонам и занижали вход вдвое.
 *
 * `retryInTok` замерен отдельно: ретрай после S-COMPANY-AI-1a продолжает тот же
 * диалог — контекст оплачивается ещё раз, новых поисков нет. После S-COMPANY-AI-1c
 * (снятие тегов цитирования до проверки формы) срабатывать он должен редко.
 *
