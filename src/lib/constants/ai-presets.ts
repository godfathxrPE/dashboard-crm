...
export const PRICE_BY_SLUG: Record<string, { in: number; out: number }> = {
  ...
  // ═══ Не-Anthropic: снапшот каталога OpenRouter `/api/v1/models` 2026-08-19 ═══
  //
  // Слаги записаны РОВНО так, как их отдаёт каталог (`deepseek/deepseek-v4-flash`,
  // `x-ai/grok-4.3`) — это те же строки, что приезжают в `ai_runs.model` из ответа
  // провайдера. Дублирующие бесвендорные формы — на случай слага из секрета: адаптер
  // дописывает вендор ТОЛЬКО `anthropic/`-моделям, поэтому бесвендорный
  // `deepseek-v4-flash` до провайдера доедет как есть только у прямого вызова.
  //
  // ⚠️ Точка в `grok-4.3` НЕ нормализуется (см. `normalizeSlug`): у не-Anthropic
  // вендоров разделитель версии — часть их собственного имени модели, и трогать
  // его значит угадывать за вендора.
  'deepseek/deepseek-v4-flash': { in: 0.083, out: 0.165 },
  'deepseek-v4-flash': { in: 0.083, out: 0.165 },
 ...
 * прямом Anthropic, а в `ai_runs.model` попадает фактический слаг из ответа
 * провайдера.
 *
 ...
 *
 * ⚠️ S-DEBT-1: ТОЧКА ВЕРСИИ СХЛОПЫВАЕТСЯ В ДЕФИС — И ТОЛЬКО У ANTHROPIC.
 * Anthropic в своём API зовёт модель `claude-haiku-4-5`, а каталог OpenRouter
 * публикует ту же модель как `claude-haiku-4.5` (сверено по `/api/v1/models`
 * 2026-08-19: дефисной формы в каталоге нет вовсе). Это не два разных слага, а
 * два написания одного, и без нормализации цена молча гасла бы на каждом
 * прогоне через OpenRouter.
 *
 * Почему нормализация НЕ общая: у чужого вендора разделитель версии — часть
 * его собственного имени, и `x-1.5` против `x-1-5` мы обязаны считать разными
 * моделями, пока не знаем обратного. Ошибка здесь несимметрична: склеенные
 * слаги дают ПРАВДОПОДОБНОЕ НЕВЕРНОЕ число в строке, подписанной словом «факт»,
 * а разошедшиеся — пустое место, по которому расход считается вручную. Поэтому
 * склейка разрешена ровно там, где расхождение написаний ДОКАЗАНО каталогом.
 *
 * Побочные формы каталога (`…:batch`, `…-fast`) в таблицу не внесены и цены не
 * получают: тариф у них другой, и это правильный `null`, а не потеря.
 */
function normalizeSlug(slug: string): string {
  const s = slug.trim().toLowerCase();
  const bare = s.startsWith(ANTHROPIC_VENDOR) ? s.slice(ANTHROPIC_VENDOR.length) : s;
  return bare.startsWith('claude-') ? bare.replace(/(\d)\.(\d)/g, '$1-$2') : bare;
}
