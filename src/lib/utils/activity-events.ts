...
export function describeEvent(entry: ActivityLog): string {
  ...
  switch (entry.event_type) {
    ...
    case 'meeting_scheduled':
      // ⚠️ НЕ «Встреча: …» — это запись ЖУРНАЛА о планировании, а сама встреча
      // приезжает в ленту отдельным событием `kind='meeting'` с тем же названием
      // (`meetingToEvent` → «Встреча: X»). Совпадавшие заголовки читались на карточке
      // компании как дубль — находка гейта S-TL-1.
      return `Запланирована встреча: ${p.title ?? ''}`;
