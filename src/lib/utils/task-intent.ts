...
const TRIGGER_RES: RegExp[] = [
  ...
  // (иконка «в задачу» на сообщении, которое само начинается с неисполненной команды).
  /^\s*\/\s*(?:задач[аиу]|task)(?![\wа-яё])\s*/i,
...
/**
 * Слэш-команда композера. Живёт здесь, а не в `MessageThread`: это чистое правило
 * разбора строки, и покрыть его тестами можно только вне клиентского компонента.
 *
 * Терпимость к вводу — не украшение, а вывод из живого смока: Олег написал в канал
 * `/Задача - …`, `/ задача - …` и `/ задача`, и две формы из трёх ушли обычным
 * сообщением. Поэтому: пробел после слэша, любой регистр, `задачу` наравне с `задача`
 * и `/task` синонимом — раскладку переключить забывают чаще, чем кажется.
 *
 * `\b` не используется (в JS он опирается на ASCII-`\w` и после кириллицы срабатывает
 * наоборот ожидаемому) — граница `(?![\wа-яё])`.
 */
const TASK_COMMAND_RE = /^\/\s*(?:задач[ау]|task)(?![\wа-яё])\s*/i;

/**
 * Остаток строки после команды, либо `null` — это не команда.
 *
 * Пустая строка в ответе значит «команда есть, текста нет»: вызывающий обязан различать
 * её и `null`, иначе `/задача` без текста молча уйдёт в канал сообщением.
 */
export function matchTaskCommand(body: string): string | null {
